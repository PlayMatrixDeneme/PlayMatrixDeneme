...
function setPatternAuthLocked(locked) {
    if (locked) {
        gameActive = false;
        isShowing = false;
        ...
        if (startBtn) {
            ...
            startBtn.innerText = "GİRİŞ YAP";
        }
        if (statusEl) statusEl.innerText = "Oynamak ve seviyene puan eklemek için giriş yap.";
        return;
    }
    if (grid) grid.style.pointerEvents = "auto";
    if (!gameActive && startBtn) {
        startBtn.disabled = false;
        startBtn.innerText = "BAŞLAT";
    }
    if (!gameActive && statusEl) statusEl.innerText = "HAZIR OL";
}

window.__PM_PATTERN_MASTER_GAME__ = {
    isActive: () => !!gameActive,
    stopForAuth: () => setPatternAuthLocked(true),
    unlockForAuth: () => setPatternAuthLocked(false)
};
