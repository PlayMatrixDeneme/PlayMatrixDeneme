...
function setSnakeStartupMessage(title, subtitle) {
    const startup = document.getElementById("startup");
    if (!startup) return;
    const children = startup.querySelectorAll("div");
    if (children[0]) children[0].innerText = title;
    if (children[1]) children[1].innerText = subtitle;
}

function stopSnakeForAuth() {
    started = false;
    ...
    setSnakeStartupMessage("GİRİŞ GEREKLİ", "GİRİŞ YAPMAK İÇİN DOKUN");
}

function unlockSnakeForAuth() {
    if (!started) {
        const startup = document.getElementById("startup");
        if (startup) startup.style.display = "flex";
        setSnakeStartupMessage("SNAKE PRO", "BAŞLAMAK İÇİN DOKUN");
    }
}

window.__PM_SNAKE_PRO_GAME__ = {
    isActive: () => !!started,
    stopForAuth: stopSnakeForAuth,
    unlockForAuth: unlockSnakeForAuth
};
