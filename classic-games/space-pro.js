...
function update() {
    ...
    if (entities.boss) {
        ...
        for (let mi = entities.mermiler.length - 1; mi >= 0; mi--) {
            ...
            if (checkHit(m, b, 4, 15, 80, 80)) {
                ...
                if (b.hp <= 0) {
                    for (let k = 0; k < 4; k++) {
                        entities.powerups.push({ x: b.x + Math.random() * 60, y: b.y + Math.random() * 40, t: 'L' });
                    }
    ...
    if (state.lives <= 0) {
        ...
        if (state.score > state.highScore) {
            localStorage.setItem("proSpaceScore", state.score);
            state.highScore = state.score;
        }
        document.getElementById("gameOver").style.display = "flex";
        setSpaceGameOverText("GAME OVER", `FINAL SCORE: ${state.score}`, "TEKRAR DENE");
...
function setSpaceGameOverText(title, scoreText, buttonText) {
    const gameOver = document.getElementById("gameOver");
    const heading = gameOver?.querySelector("h1");
    const finalScore = document.getElementById("finalScore");
    if (heading) heading.innerText = title;
    if (finalScore) finalScore.innerText = scoreText;
    if (restartBtn) restartBtn.innerText = buttonText;
}

function stopSpaceForAuth() {
    state.running = false;
    const gameOver = document.getElementById("gameOver");
    if (gameOver) gameOver.style.display = "flex";
    setSpaceGameOverText("GİRİŞ GEREKLİ", "OYNAMAK VE XP KAZANMAK İÇİN GİRİŞ YAP", "GİRİŞ YAP");
}

function unlockSpaceForAuth() {
    setSpaceGameOverText("GAME OVER", `FINAL SCORE: ${state.score}`, "TEKRAR DENE");
    if (!state.running) init();
}

window.__PM_SPACE_PRO_GAME__ = {
    isActive: () => !!state.running,
    stopForAuth: stopSpaceForAuth,
    unlockForAuth: unlockSpaceForAuth,
    startWhenReady: init
};

const handleCtrl = (id, type) => {
    const el = document.getElementById(id);
    el.addEventListener("touchstart", (e) => { e.preventDefault(); audio.init(); if (type === 'L') state.moveLeft = true; else state.moveRight = true; }, { passive: false });
    el.addEventListener("touchend", (e) => { e.preventDefault(); if (type === 'L') state.moveLeft = false; else state.moveRight = false; }, { passive: false });
