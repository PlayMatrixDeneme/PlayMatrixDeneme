...
const { deleteDirectMessageSearchIndex } = require('../utils/chatSearchIndex');
...
const colMonthlyRewardRuns = () => db.collection('monthly_reward_runs');

const MONTHLY_REWARD_STATE = Object.freeze({
...
async function hasMonthlyRewardsRecordedForMonth(monthKey = '') {
  ...
  const runSnap = await colMonthlyRewardRuns().doc(safeMonthKey).get().catch(() => null);
  return !!(runSnap?.exists && ['completed', 'RESET_DONE'].includes(String(runSnap.data()?.status || '')));
}

async function markMonthlyRewardRunFailed(ref, summary = {}) {
  if (!ref) return;
  const now = nowMs();
  await ref.set({
    status: 'failed',
    failedAt: now,
    updatedAt: now,
    lastError: cleanStr(summary.error || summary.message || '', 500),
    summary: {
      rewardMonthKey: cleanStr(summary.rewardMonthKey || '', 32) || null,
      currentPeriodKey: cleanStr(summary.currentPeriodKey || '', 32) || null
    }
  }, { merge: true }).catch(() => null);
}

async function beginMonthlyRewardRun(monthKey = '', resetMeta = {}) {
  const safeMonthKey = String(monthKey || '').trim();
  if (!safeMonthKey) throw new Error('MONTHLY_REWARD_MONTH_KEY_REQUIRED');
  const ref = colMonthlyRewardRuns().doc(safeMonthKey);
  const now = nowMs();
  return db.runTransaction(async (tx) => {
    const snap = await tx.get(ref);
    const data = snap.exists ? (snap.data() || {}) : {};
    if (['completed', MONTHLY_REWARD_STATE.RESET_DONE].includes(String(data.status || ''))) {
      return { proceed: false, ref, data, duplicated: true };
    }
    tx.set(ref, {
      monthKey: safeMonthKey,
      status: MONTHLY_REWARD_STATE.WINDOW_OPEN,
      resetPeriodKey: String(resetMeta.currentPeriodKey || '').trim() || null,
      previousPeriodKey: String(resetMeta.previousPeriodKey || '').trim() || null,
      rewardWindowOpenedAt: safeNum(resetMeta.resetAt, 0),
      rewardWindowClosesAt: safeNum(resetMeta.monthlyWindowClosesAt, 0),
      startedAt: data.startedAt || now,
      updatedAt: now,
      timestamp: admin.firestore.FieldValue.serverTimestamp()
    }, { merge: true });
    return { proceed: true, ref, data, duplicated: false };
  });
}

async function finalizeMonthlyRewardRun(ref, summary = {}) {
  if (!ref) return;
  const now = nowMs();
  await ref.set({
    status: MONTHLY_REWARD_STATE.RESET_DONE,
    rewardState: MONTHLY_REWARD_STATE.RESET_DONE,
    awardedCount: Math.max(0, Math.floor(safeNum(summary.awardedCount, 0))),
    eligibleCount: Math.max(0, Math.floor(safeNum(summary.eligibleCount, 0))),
    resetApplied: summary.resetApplied !== false,
    completedAt: now,
    updatedAt: now,
    summary: {
      rewardMonthKey: String(summary.rewardMonthKey || '').trim() || null,
      currentPeriodKey: String(summary.currentPeriodKey || '').trim() || null
    }
  }, { merge: true });
}
  ...
  try {
    ...
    if (!resetMeta.isMonthlyRewardWindowOpen) {
      return await releaseJobLock(lock.ref, buildJobWindowState(lock.data, resetMeta, {
        ...
        monthlyRewardState: MONTHLY_REWARD_STATE.WINDOW_CLOSED,
        waitReason: 'monthly_reward_window_closed'
      }));
    }

    monthlyRun = await beginMonthlyRewardRun(rewardMonthKey, resetMeta);
    if (!monthlyRun.proceed) {
      return await releaseJobLock(lock.ref, buildJobWindowState(lock.data, resetMeta, {
        lastProcessedPeriodKey: currentPeriodKey,
        lastRewardMonthKey: rewardMonthKey,
        alreadyRecordedForMonth: true,
        waitReason: null
      }));
    }
    ...
    await monthlyRun.ref.set({ rewardState: MONTHLY_REWARD_STATE.REWARD_GRANTED, rewardedAt: nowMs(), updatedAt: nowMs() }, { merge: true });
    await resetMonthlyActivityForNewPeriod(currentPeriodKey);
    await finalizeMonthlyRewardRun(monthlyRun.ref, { awardedCount, eligibleCount: eligibleRewards.length, rewardMonthKey, currentPeriodKey, resetApplied: true });
    await releaseJobLock(lock.ref, buildJobWindowState(lock.data, resetMeta, {
      lastProcessedPeriodKey: currentPeriodKey,
      lastRewardMonthKey: rewardMonthKey,
      monthlyRewardState: MONTHLY_REWARD_STATE.RESET_DONE,
    ...
    await markMonthlyRewardRunFailed(monthlyRun?.ref, {
...
async function processActivityResetIfNeeded() {
  ...
  try {
    ...
    const rewardMonthKey = resetMeta.previousPeriodKey || getPreviousActivityPeriodKey(currentPeriodKey);
    const monthlyRewardsCompleted = await hasMonthlyRewardsRecordedForMonth(rewardMonthKey);
    if (!monthlyRewardsCompleted) {
      return await releaseJobLock(lock.ref, buildJobWindowState(lock.data, resetMeta, {
        deferredPeriodKey: currentPeriodKey,
        deferredRewardMonthKey: rewardMonthKey,
        deferredUntilWindow: resetMeta.monthlyWindowClosesAt,
        waitReason: 'monthly_rewards_pending'
      }));
    }
...
async function cleanupStaleData() {
  try {
    ...
    for (const msgDoc of directExpiredSnap.docs || []) {
      ...
      const participants = Array.isArray(chatData.participants) && chatData.participants.length
        ? chatData.participants
        : cleanStr(chatRef.id || '', 240).split('_').filter(Boolean);
      if (safeNum(data.deletedAt, 0) > 0 && createdAt < oldDirectChatHardPurge) {
        stats.directMessagesPurged += 1;
        indexCleanupTasks.push(deleteDirectMessageSearchIndex({ chatId: chatRef.id, messageId: msgDoc.id, participants }).catch(() => null));
    ...
    if (indexCleanupTasks.length > 0) await Promise.allSettled(indexCleanupTasks);
    ...
    writeLine('debug', 'cron_chat_retention_cleanup_completed', summary);
  } catch (error) {
