...
const AUTO_CASHOUT_MAX_RETRIES = Math.max(5, Math.floor(Number(process.env.AUTO_CASHOUT_MAX_RETRIES || 25)));
const AUTO_CASHOUT_MAX_RETRY_DELAY_MS = Math.max(AUTO_CASHOUT_RETRY_DELAY_MS, Math.floor(Number(process.env.AUTO_CASHOUT_MAX_RETRY_DELAY_MS || 30000)));
...
async function settleSingleCrashLoss({ uid = '', bet = {}, boxKey = '', roundId = '', crashPoint = 0, settledAt = nowMs() } = {}) {
  ...
  return db.runTransaction(async (tx) => {
    ...
    recordGameLedgerInTransaction(tx, {
      uid,
      gameType: 'crash',
      source: 'crash_crashed_loss',
      referenceId: bet.betId,
      roomId: roundId,
      roundId,
      betId: bet.betId,
      amount: 0,
      stake: Math.max(0, Math.floor(safeNum(bet.bet ?? bet.amount ?? betDoc.amount, 0))),
      payout: 0,
      resultCode: GAME_RESULT_CODES.CRASH_CRASHED_LOSS,
      settlementStatus: GAME_SETTLEMENT_STATUS.SETTLED,
      actorUid: uid,
      subjectUid: uid,
      meta: { box: bet.box || (boxKey === 'box2' ? 2 : 1), crashPoint: safeFloat(crashPoint), reason: 'crashed_loss' },
      idempotencyKey: `crash:${bet.betId}:crashed_loss:result`
    });
    ...
    return {
      uid,
      betId: bet.betId,
      amount: Math.max(0, Math.floor(safeNum(bet.bet ?? bet.amount ?? betDoc.amount, 0))),
      payout: 0,
      resultCode: GAME_RESULT_CODES.CRASH_CRASHED_LOSS,
      cashoutMult: 0,
      crashPoint: safeFloat(crashPoint),
...
async function settleCrashRoundLosses(roundId = '', crashPoint = 0) {
  ...
  const candidatesByBetId = new Map();
  const addCandidate = (candidate = {}) => {
    const betId = cleanStr(candidate?.bet?.betId || candidate?.betId || '', 180);
    const uid = cleanStr(candidate?.uid || candidate?.bet?.uid || '', 160);
    if (!betId || !uid) return;
    candidatesByBetId.set(betId, {
      uid,
      bet: {
        ...(candidate.bet || {}),
        betId,
        uid,
        bet: safeFloat(candidate?.bet?.bet ?? candidate?.bet?.amount ?? candidate.amount ?? 0),
        amount: safeFloat(candidate?.bet?.amount ?? candidate?.bet?.bet ?? candidate.amount ?? 0),
        box: safeNum(candidate?.bet?.box ?? candidate.box, candidate.boxKey === 'box2' ? 2 : 1),
        roundId: safeRoundId
      },
      boxKey: candidate.boxKey || `box${safeNum(candidate?.bet?.box ?? candidate.box, 1) === 2 ? 2 : 1}`,
      roundId: safeRoundId,
      crashPoint,
      settledAt
    });
  ...
  Object.entries(engineState.crashState.players || {}).forEach(([uid, player]) => {
    ['box1', 'box2'].forEach((boxKey) => {
      ...
      addCandidate({ uid, bet, boxKey });
    });
  });

  try {
    const snap = await colCrashBets()
      .where('roundId', '==', safeRoundId)
      .where('status', '==', 'active')
      .limit(1000)
      .get();
    for (const doc of snap.docs || []) {
      const data = doc.data() || {};
      if (cleanStr(data.settlementStatus || '', 24) !== GAME_SETTLEMENT_STATUS.ACTIVE) continue;
      if (data.cashed === true) continue;
      addCandidate({
        uid: cleanStr(data.uid || '', 160),
        betId: doc.id,
        amount: safeFloat(data.amount || 0),
        box: safeNum(data.box, 1),
        bet: {
          betId: doc.id,
          uid: cleanStr(data.uid || '', 160),
    ...
    console.error('settleCrashRoundLosses query error:', error?.message || error);
  }

  const candidates = Array.from(candidatesByBetId.values());
...
function enqueueAutoCashoutPersist(uid, boxKey, betData, finalMult, finalWin) {
  ...
  autoCashoutQueue.set(betData.betId, {
    uid,
    boxKey,
    betId: betData.betId,
    roundId: cleanStr(betData.roundId || engineState.crashState.roundId || '', 160),
    stake: safeFloat(betData.bet ?? betData.amount ?? 0),
    box: safeNum(betData.box || (boxKey === 'box2' ? 2 : 1), 1),
    finalMult,
    finalWin,
    retryCount: 0,
    queuedAt: nowMs(),
...
function getNextAutoCashoutRetryDelay() {
  const now = nowMs();
  const nextDue = Array.from(autoCashoutQueue.values())
    .map((item) => safeNum(item.nextRetryAt, 0))
    .filter((ts) => ts > now)
    .sort((a, b) => a - b)[0];
  return nextDue ? Math.max(0, nextDue - now) : 0;
}

function computeAutoCashoutRetryDelay(retryCount = 0) {
  const exp = Math.max(0, Math.min(AUTO_CASHOUT_MAX_RETRIES, Math.floor(safeNum(retryCount, 0))));
  return Math.min(AUTO_CASHOUT_MAX_RETRY_DELAY_MS, AUTO_CASHOUT_RETRY_DELAY_MS * Math.pow(2, Math.min(7, exp)));
}

function scheduleAutoCashoutFlush(delayMs = 0) {
  ...
  setTimeout(() => {
      flushAutoCashoutQueue().catch(e => console.error('flushAutoCashoutQueue error:', e)).finally(() => {
          ...
          if (autoCashoutQueue.size > 0) scheduleAutoCashoutFlush(getNextAutoCashoutRetryDelay());
      });
  }, Math.max(0, safeNum(delayMs, 0)));
}

function markAutoCashoutPersistFailed(item = {}, reason = 'auto_cashout_persist_failed') {
  const retryCount = Math.min(AUTO_CASHOUT_MAX_RETRIES, Math.floor(safeNum(item.retryCount, 0) + 1));
  item.retryCount = retryCount;
  item.lastError = cleanStr(reason || 'auto_cashout_persist_failed', 120);
  item.lastRetryAt = nowMs();
  item.nextRetryAt = item.lastRetryAt + computeAutoCashoutRetryDelay(retryCount);
  autoCashoutQueue.set(item.betId, item);
  const player = engineState.crashState.players[item.uid];
  if (player && player[item.boxKey]) {
    player[item.boxKey].settlementPending = true;
    player[item.boxKey].cashingOut = true;
  }
}
...
async function flushAutoCashoutQueue() {
  if (!isCrashDbReady()) {
    ...
    Array.from(autoCashoutQueue.values()).forEach((item) => markAutoCashoutPersistFailed(item, 'FIREBASE_NOT_READY'));
    engineState.triggerUpdate();
    return;
  }

  const now = nowMs();
  const entries = Array.from(autoCashoutQueue.values())
    .filter((item) => safeNum(item.nextRetryAt, 0) <= now)
    .slice(0, AUTO_CASHOUT_BATCH_SIZE);
      ...
      try {
          ...
          const settlement = await db.runTransaction(async (tx) => {
            ...
            recordGameLedgerInTransaction(tx, {
              uid: item.uid,
              gameType: 'crash',
              source: 'crash_auto_cashout_payout',
              referenceId: item.betId,
              roomId: cleanStr(betDoc.roundId || '', 160),
              roundId: cleanStr(betDoc.roundId || '', 160),
              betId: item.betId,
              amount: Math.floor(item.finalWin),
              stake: betDoc.amount,
              ...
              actorUid: item.uid,
              subjectUid: item.uid,
              meta: { box: betDoc.box, cashoutMult: item.finalMult, reason: 'cashout_auto' },
              idempotencyKey: `crash:${item.betId}:cashout_auto:payout`
            });
          ...
          if (player && player[item.boxKey]) {
            ...
            player[item.boxKey].settlementPending = false;
          ...
          markAutoCashoutPersistFailed(item, e?.message || e || 'auto_cashout_failed');
          engineState.triggerUpdate();
      }
  }
}
...
function processAutoCashout(uid, boxKey, targetMult) {
  ...
  betData.cashingOut = true; betData.settlementPending = true; betData.cashed = true; betData.win = finalWin; betData.cashoutMult = finalMult;
  engineState.triggerUpdate();
