...
function extractAdminClientKey(req = {}) {
  return cleanStr(req.headers?.['x-admin-client-key'] || '', 2000);
}

function verifyAdminClientGate(req = {}, context = {}) {
  const key = extractAdminClientKey(req);
  if (!key) return { ok: false, code: 'ADMIN_CLIENT_KEY_REQUIRED' };

  const state = verifyClientGateKey(key);
  if (!state.ok) return { ok: false, code: state.code || 'ADMIN_CLIENT_KEY_INVALID' };
  ...
  const expectedSessionId = cleanStr(req.user?.sessionId || '', 160);

  if (!expectedUid || payload.uid !== expectedUid) return { ok: false, code: 'ADMIN_CLIENT_KEY_UID_MISMATCH' };
  if (expectedEmail && normalizeEmail(payload.email || '') !== expectedEmail) return { ok: false, code: 'ADMIN_CLIENT_KEY_EMAIL_MISMATCH' };
  if (!expectedSessionId || cleanStr(payload.sessionId || '', 160) !== expectedSessionId) {
    return { ok: false, code: 'ADMIN_CLIENT_KEY_SESSION_MISMATCH' };
  }
    ...
    await verifyAuth(req, res, async () => {
      ...
      if (!gate.ok) {
        return res.status(403).json({
          ok: false,
          code: gate.code,
          error: 'Yönetici güvenlik anahtarı doğrulanamadı. Lütfen ikinci ve üçüncü güvenlik doğrulamasını tamamlayın.',
