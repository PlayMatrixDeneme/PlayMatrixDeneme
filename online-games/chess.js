...
Object.assign(window, { closeConfirmModal, showHowToPlay, showMatrixModal });
    ...
    async function fetchBalance() {
      try { 
        ...
        return hydrateGameTopbar(res, { balanceIds: ['ui-balance', 'uiBalance'] });
      } catch(e) { console.warn('[PlayMatrix:Satranc] profile load skipped', e); return null; }
    }
    ...
    window.createRoom = async () => {
      try {
        ...
        if (!(res?.ok && res.room?.id)) throw new Error(res?.error || 'Oda kurulamadı.');
    ...
    window.joinRoom = async (id) => {
      try {
        ...
        if (!(res?.ok && res.room?.id)) throw new Error(res?.error || 'Odaya katılınamadı.');
        notifyGameAccountMutation(res);
        enterGame(res.room);
      } catch(e) { showMatrixModal("Hata", e.message, "error"); }
    };
    ...
    function enterGame(roomData) {
      ...
      setActiveGameRoom('chess', String(roomData.id || ''));
    ...
    async function pollGameState() {
      ...
      try {
        ...
        if (e.message === "Oda bulunamadı.") {
          ...
          showGameNotice('Oyun durumu güncellenemedi. Tekrar deneniyor.', 'warning', 'Lobiye Dön', () => { clearActiveGameRoom('chess'); window.location.reload(); });
        }
      }
    }
    ...
    function applyFramedAvatar(avatarId, frameId, avatarUrl, selectedFrameLevel) {
      ...
      frameEl.src = `/frames/frame-${frameIndex}.png`;
      frameEl.dataset.fallback = `/frames/frame-${frameIndex}.png`;
    }
    ...
    window.resignGame = () => {
      showConfirmModal("Teslim Ol", "Teslim olmak istediğinize emin misiniz? (Rakibe galibiyet işlenir)", async () => {
        try { const res = await fetchAPI('/api/chess/resign', 'POST', { roomId: currentRoomId }); notifyGameAccountMutation(res); } catch(e) {}
      });
    };
