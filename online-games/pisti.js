...
import { escapeHTML } from "../public/js/games/shared/dom-sanitize.js";
import { scheduleGameBoot } from "../public/js/games/shared/room-state.js";
import { hydrateGameTopbar } from "../public/js/games/shared/topbar.js";
import { applyGameAccountSnapshot, notifyGameAccountMutation } from "../public/js/games/shared/reward-client.js";
import { setActiveGameRoom, clearActiveGameRoom } from "../public/js/games/shared/navigation.js";
import { emitSocketIfConnected } from "../public/js/games/shared/socket-events.js";
...
async function handleHostInviteAcceptedRedirect(payload = {}) {
  try {
    ...
    if (gameType.includes('chess') || gameType.includes('satran')) {
      setActiveGameRoom('chess', roomId);
      window.location.href = '/online-games/chess.html';
      return true;
    }
    if (gameType.includes('pisti') || gameType.includes('pişti')) {
      setActiveGameRoom('pisti', roomId);
      window.location.href = '/online-games/pisti.html';
      return true;
    }
  } catch (_) {}
  return false;
...
function switchCreateTab(tab = 'open') {
  ...
  if (action) action.textContent = next === 'private' ? 'ÖZEL MASA KUR' : 'MASA KUR';
}
...
function dismissIntro() {
  ...
  elStudioIntro.dataset.bootCompleted = '1';
...
async function fetchProfile(){ 
    ...
    applyGameAccountSnapshot(res);
    const snapshot = hydrateGameTopbar(res);
    return { balance: snapshot.balance, accountLevel: snapshot.accountLevel, accountProgress: snapshot.accountProgress };
}
...
function resetToLobby(){ 
  if (currentRoomId) emitSocketIfConnected(socket, 'pisti:leave', currentRoomId);
...
window.submitCreateRoomAction = async () => {
  ...
  if (tab === 'open') {
      try{ const res = await fetchAPI('/api/pisti-online/play-open', 'POST', { mode, bet, createOnly: true }); if(res?.ok && res.room?.id) { notifyGameAccountMutation(res); await enterGame(res.room.id); } else throw new Error(res.error || 'Masa oluşturulamadı.'); } catch(e){ showMatrixModal("Reddedildi", e.message, "error"); } finally { isProcessing = false; }
  } else {
      ...
      try{ const res = await fetchAPI('/api/pisti-online/create-private', 'POST', {mode, bet, roomName, password}); if(res?.ok && res.room?.id) { notifyGameAccountMutation(res); await enterGame(res.room.id); } else throw new Error(res.error || 'Özel masa oluşturulamadı.'); } catch(e){ showMatrixModal("Reddedildi", e.message, "error"); } finally { isProcessing = false; }
  }
};

window.quickJoinPistiTable = async () => {
  if (isProcessing) return;
  isProcessing = true;
  try {
    const mode = document.getElementById('roomModeSelect')?.value || '2-52';
    const bet = parseInt(document.getElementById('roomBetInput')?.value || '1000', 10) || 1000;
    const res = await fetchAPI('/api/pisti-online/play-open', 'POST', { mode, bet });
    if (res?.ok && res.room?.id) {
      notifyGameAccountMutation(res);
      await enterGame(res.room.id);
      return;
    }
    throw new Error(res?.error || 'Müsait masa bulunamadı veya masa açılamadı.');
  } catch (e) {
    showMatrixModal('Reddedildi', e.message || 'Hızlı masa bulunamadı.', 'error');
  } finally {
    isProcessing = false;
  }
};
...
window.joinRoom = async (id, password='') => { 
  if (isProcessing) return; isProcessing = true;
  try{ const res=await fetchAPI('/api/pisti-online/join','POST',{roomId:id, password}); if(res?.ok && res.room) { notifyGameAccountMutation(res); await enterGame(res.room.id || id); } else throw new Error(res.error || 'Masaya katılınamadı.'); }catch(e){showMatrixModal("Reddedildi",e.message,"error");} finally { isProcessing = false; } 
};
...
async function enterGame(id){
  ...
  if(socketReady) emitSocketIfConnected(socket, 'pisti:join', id);
...
function buildFramedAvatarHTML(avatarUrl, selectedFrame, imageClass, wrapperClass = 'pm-game-avatar-shell--main') {
  ...
  const frameHtml = frameIndex > 0
    ? `<img src="/frames/frame-${frameIndex}.png" class="pm-game-frame frame-${frameIndex}" alt="" aria-hidden="true" data-fallback="/frames/frame-${frameIndex}.png">`
    : '';
