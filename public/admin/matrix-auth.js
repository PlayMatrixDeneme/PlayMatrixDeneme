import { preventUserInterference, initMatrixRain, clearSecurityKey, adminFetch, resolveAdminUrl, resolveAdminBackendUrl } from './matrix-core.js';
...
async function verifyName() {
  ...
  try {
    const out = await adminFetch('/api/auth/admin/matrix/step-name', { method: 'POST', body: JSON.stringify({ ticket: state.ticket, adminName, next: '/public/admin/admin.html' }) });
    setStatus('nameStatus', 'Güvenli yönetici oturumu başlatıldı. Panel açılıyor...', 'ok');
    state.redirecting = true;
    activateStep(4);
    const redirectTarget = out?.redirectTo;
    const target = redirectTarget ? resolveAdminBackendUrl(out?.redirectTo) : DASHBOARD_URL;
