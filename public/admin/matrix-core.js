...
export function setSecurityKey(_value = '') {
  return false;
}
...
export function getLastAdminBase() {
  return normalizeAdminBase(lastSuccessfulAdminBase || '');
}

export function resolveAdminBackendUrl(target = '/admin/admin.html') {
  const raw = String(target || '/admin/admin.html').trim();
  if (/^https?:\/\//i.test(raw)) return raw;
  const base = normalizeAdminBase(lastSuccessfulAdminBase)
    || normalizeAdminBase(window.__PM_RUNTIME?.apiBase)
    || normalizeAdminBase(window.__PM_STATIC_RUNTIME_CONFIG__?.apiBase)
    || normalizeAdminBase(window.__PLAYMATRIX_API_URL__)
    || normalizeAdminBase(PM_ADMIN_BACKEND_FALLBACK)
    || normalizeAdminBase(window.location.origin);
  try {
    return new URL(raw.startsWith('/') ? raw : `/${raw}`, base || window.location.origin).toString();
  } catch (_) {
    return raw || '/admin/admin.html';
  }
}
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    ...
    try {
      ...
      if (response.ok && payload?.ok !== false) {
        if (base) {
          lastSuccessfulAdminBase = base;
          if (window.__PM_API__?.setApiBase) window.__PM_API__.setApiBase(base);
        }
        return payload;
      }
