import { preventUserInterference, initMatrixRain, adminFetch, clearSecurityKey, money, formatWhen, resolveAdminUrl } from './matrix-core.js';
    ...
    <div class="admin-app">
      ...
      <div class="layout-hero">
        ...
        <section class="panel stack">
          <div>
            ...
            <p class="lead">maintenance modu açık olan oyunlar anasayfadan girişte otomatik olarak bakım sayfasına yönlenir.</p>
          </div>
      ...
      <div class="layout-grid-2">
        <section class="panel stack">
          ...
          <div class="confirm-box"><strong>Yazılı Onay</strong><input id="promoConfirm" type="text" placeholder="ONAYLIYORUM" /></div>
...
function buildMaintenanceButton(key, label, enabled) {
  ...
  body.append(el('strong', '', label), el('span', '', enabled ? 'maintenanceda' : 'Korumada'));
...
async function loadDashboard() {
  ...
  const resetLabels = [
    ...
    ['accountXp', 'XP'], ['activityScore', 'Aktiflik Puanı'], ['selectedFrame', 'Seçili frames']
  ];
...
async function handleAction(action) {
  try {
    ...
    if (action === 'save-maintenance') {
      ...
      setStatus('maintenanceStatus', 'maintenance modu ayarları kaydedildi.', 'ok');
      return;
    }
    ...
    if (action === 'promo-create') {
      ...
      const confirmText = document.getElementById('promoConfirm')?.value.trim().toUpperCase();
      if (confirmText !== 'ONAYLIYORUM') throw new Error('Yazılı onay gerekli.');
      ...
      await adminFetch('/api/admin/matrix/promo-codes', { method: 'POST', body: JSON.stringify({ code, amount, durationHours, usageLimit, onePerAccount, description, confirmText }) });
