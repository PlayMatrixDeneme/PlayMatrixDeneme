  ...
  return `
    <div class="admin-app">
      ...
      <div class="layout-grid-2">
        ...
        <section class="panel stack">
          <div>
            <h2>MARKET FİYAT YÖNETİMİ</h2>
            <p class="lead">Çerçeve, istatistik arka planı, chat skin, nameplate, rozet ve isim efekti fiyatları buradan güncellenir.</p>
          </div>
          <div class="field-grid">
            <div class="field"><label for="marketItemId">Market Ürün ID</label><input id="marketItemId" type="text" placeholder="frame_market_1" /></div>
            <div class="field"><label for="marketPrice">Yeni MC Fiyatı</label><input id="marketPrice" type="number" min="0" step="1" placeholder="25000" /></div>
            <div class="field"><label for="marketActive">Aktiflik</label><select id="marketActive"><option value="true">Aktif</option><option value="false">Pasif</option></select></div>
            <div class="field"><label for="marketReason">Gerekçe</label><input id="marketReason" type="text" placeholder="Fiyat güncelleme gerekçesi" /></div>
          </div>
          <div class="action-row"><button id="saveMarketPriceBtn" type="button">MARKET ÜRÜNÜNÜ GÜNCELLE</button></div>
          <div class="status" id="marketStatus"></div>
          <div class="table-wrap"><table><thead><tr><th>ID</th><th>Tip</th><th>Ad</th><th>Fiyat</th><th>Durum</th></tr></thead><tbody id="marketRows"></tbody></table></div>
        </section>
...
async function loadDashboard() {
  const [dashboard, promos, issues, market] = await Promise.all([
    ...
    adminFetch('/api/admin/market/items').catch(() => ({ items: [] }))
  ]);
...
async function handleAction(action) {
  try {
    ...
    if (action === 'market-price') {
      const itemId = document.getElementById('marketItemId')?.value.trim();
      const priceMcExact = Number(document.getElementById('marketPrice')?.value || 0);
      const active = String(document.getElementById('marketActive')?.value || 'true') === 'true';
      const reason = document.getElementById('marketReason')?.value.trim();
      if (!itemId) throw new Error('Market ürün ID gerekli.');
      if (priceMcExact < 0) throw new Error('Geçerli MC fiyatı gerekli.');
      if (!reason) throw new Error('Gerekçe gerekli.');
      await adminFetch(`/api/admin/market/items/${encodeURIComponent(itemId)}`, { method: 'PATCH', body: JSON.stringify({ priceMcExact, active, reason }) });
      setStatus('marketStatus', 'Market ürünü güncellendi.', 'ok');
      return loadDashboard();
    }
    ...
    const map = {
      reset: 'resetStatus', 'save-maintenance': 'maintenanceStatus', 'reward-user': 'rewardStatus', 'reward-all': 'rewardAllStatus', 'promo-create': 'promoStatus', 'market-price': 'marketStatus'
    };
...
root.addEventListener('click', (event) => {
  ...
  if (button.id === 'saveMarketPriceBtn') return handleAction('market-price');
