...
(() => {
  ...
  function buildHTML({ avatarUrl = '', level = 0, exactFrameIndex = null, sizePx = 45, extraClass = '', imageClass = 'pm-avatar-img', wrapperClass = 'pm-avatar', alt = 'Oyuncu', sizeTag = '' } = {}) {
    ...
    const frameHtml = frameIndex > 0
      ? `<img src="/Cerceve/frame-${frameIndex}.png" class="pm-frame-image pm-avatar-shell__frame frame-${frameIndex}" alt="" aria-hidden="true" loading="lazy" decoding="async" draggable="false" data-frame-index="${frameIndex}" data-frame-level="${normalizedLevel}" data-fallback="/Cerceve/frame-${frameIndex}.png">`
      : '';
  ...
  function createNode(options = {}) {
    ...
    if (frameIndex > 0) {
      const frame = createImage({
        ...
        alt: '',
        fallback: `/Cerceve/frame-${frameIndex}.png`,
        ariaHidden: true
      });
