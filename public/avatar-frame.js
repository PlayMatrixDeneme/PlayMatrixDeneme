...
(() => {
  ...
  function buildHTML({ avatarUrl = '', level = 0, exactFrameIndex = null, sizePx = 45, extraClass = '', imageClass = 'pm-avatar-img', wrapperClass = 'pm-avatar', alt = 'Oyuncu', sizeTag = '' } = {}) {
    ...
    const frameHtml = frameIndex > 0
      ? `<img src="/frames/frame-${frameIndex}.png" class="pm-frame-image pm-avatar-shell__frame frame-${frameIndex}" alt="" aria-hidden="true" loading="lazy" decoding="async" draggable="false" data-frame-index="${frameIndex}" data-frame-level="${normalizedLevel}" data-fallback="/frames/frame-${frameIndex}.png">`
      : '';
  ...
  function createNode(options = {}) {
    ...
    if (frameIndex > 0) {
      const frame = createImage({
        ...
        alt: '',
        fallback: `/frames/frame-${frameIndex}.png`,
        ariaHidden: true
      });
  ...
  function normalizeAvatarFrameContract({
    avatarUrl = '',
    frameId = 0,
    size = 56,
    level = 1,
    ...
    const normalizedFrame = normalizeLevel(frameId || 0);
    const safeLevel = normalizeLevel(level || normalizedFrame || 0);
    const safeSize = Math.max(20, Math.min(220, Math.floor(Number(size) || 56)));
    return Object.freeze({
      ...
      alt: String(alt || 'Oyuncu'),
      extraClass: String(extraClass || '')
    });
  }

  function AvatarFrame(input = {}) {
    const contract = normalizeAvatarFrameContract(input);
    const node = createNode({
      avatarUrl: contract.avatarUrl,
      level: contract.frameId,
      sizePx: contract.size,
      alt: contract.alt,
      extraClass: ['pm-avatar-frame-contract', contract.extraClass, contract.locked ? 'is-locked' : '', contract.interactive ? 'is-interactive' : ''].filter(Boolean).join(' '),
      sizeTag: String(input.sizeTag || 'contract')
    });
    node.dataset.pmAvatarFrameContract = 'v1';
    node.dataset.frameId = String(contract.frameId);
    node.dataset.frameLocked = contract.locked ? 'true' : 'false';
    node.dataset.interactive = contract.interactive ? 'true' : 'false';
    if (contract.locked) node.setAttribute('aria-disabled', 'true');
    return node;
  }

  function mountAvatarFrame(target, input = {}) {
    const host = typeof target === 'string' ? document.getElementById(target) : target;
    if (!host) return null;
    const node = AvatarFrame(input);
