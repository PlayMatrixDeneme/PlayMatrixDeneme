/* PlayMatrix FAZ 3: Crash application module extracted from HTML shell. */
    ...
    import { scheduleGameBoot } from "../shared/room-state.js";
    import { hydrateGameTopbar } from "../shared/topbar.js";
    import { applyGameAccountSnapshot, notifyGameAccountMutation } from "../shared/reward-client.js";
        ...
        const avatarHtml = window.PMAvatar && typeof window.PMAvatar.buildHTML === 'function'
          ...
          : (() => {
              const frameHtml = frameIndex > 0
                ? `<img src="/frames/frame-${frameIndex}.png" class="t-frame frame-${frameIndex}" alt="" aria-hidden="true" data-fallback="/frames/frame-${frameIndex}.png">`
                : '';
...
function setBootActions({ showEnter = false, showRetry = false, enterLabel = 'CRASH OYNA', actionMode = 'continue' } = {}) {
    ...
    async function fetchBootProfile() {
        ...
        applyGameAccountSnapshot(d);
        const snapshot = hydrateGameTopbar(d);
        currentBalance = safeFloat(snapshot.balance);
        if(d.user) {
    ...
    async function placeBet(box, silent = false) {
        ...
        notifyGameAccountMutation({ ...payload, balance: payload?.balance ?? currentBalance });
    ...
    async function cashOut(box) {
        ...
        notifyGameAccountMutation({ ...payload, balance: payload?.balance ?? currentBalance, winAmount: bet.win, cashoutMult: bet.cashoutMult });
        if (elUiBalance) elUiBalance.innerText = currentBalance.toLocaleString('tr-TR', {minimumFractionDigits: 2, maximumFractionDigits: 2});
    ...
    function pmRtGameHref(gameKey = '', roomId = '') {
        ...
        if (gameKey === 'chess') return `/online-games/chess.html?room=${safeRoomId}`;
        if (gameKey === 'pisti') return `/online-games/pisti.html?room=${safeRoomId}`;
        return '/online-games/crash.html';
    }
