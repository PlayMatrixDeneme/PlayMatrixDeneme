(function () {
  ...
  const GAMES = [
    { name: 'Crash', category: 'online', access: 'auth', url: '/online-games/crash', color: '69,162,255', icon: 'fa-arrow-trend-up', desc: 'Gerçek para içermeyen, refleks ve zamanlama odaklı hızlı tempo multiplier oyunu.', tags: ['Canlı Oyun', 'Rekabet', 'Hızlı Tur'] },
    { name: 'Satranç', category: 'online', access: 'auth', url: '/online-games/chess', color: '104,178,255', icon: 'fa-chess', desc: 'Klasik satranç deneyimini modern arayüz ve giriş tabanlı rekabet akışıyla oyna.', tags: ['PvP', 'Strateji', 'Arena'] },
    { name: 'Pişti', category: 'online', access: 'auth', url: '/online-games/pisti', color: '93,95,254', icon: 'fa-layer-group', desc: 'Kart takibi ve tempo yönetimi isteyen online pişti deneyimi.', tags: ['Kart', 'Online', 'Klasik'] },
    { name: 'Pattern Master', category: 'classic', access: 'auth', url: '/classic-games/pattern-master', color: '97,220,176', icon: 'fa-shapes', desc: 'Dikkat ve görsel hafıza odaklı ücretsiz pattern oyunu.', tags: ['Ücretsiz', 'Zeka', 'Refleks'] },
    { name: 'Space Pro', category: 'classic', access: 'auth', url: '/classic-games/space-pro', color: '103,170,255', icon: 'fa-user-astronaut', desc: 'Tarayıcıda anında açılan hafif ve hızlı klasik arcade uzay oyunu.', tags: ['Arcade', 'Retro', 'Ücretsiz'] },
    { name: 'Snake Pro', category: 'classic', access: 'auth', url: '/classic-games/snake-pro', color: '85,214,140', icon: 'fa-wave-square', desc: 'Retro hisli, akıcı ve ücretsiz snake deneyimi.', tags: ['Retro', 'Arcade', 'Ücretsiz'] }
  ];
  ...
  function renderGameCard(game) {
    ...
    button.append(text('span', '', game.access === 'auth' ? (hasRuntimeUser() ? 'Oyunu Aç' : 'Giriş Yap') : 'Hemen Oyna'), icon('fa-arrow-right'));
    if (game.access === 'auth') {
    ...
    if (game.name === 'Satranç' || game.name === 'Pişti') {
      const actionGroup = document.createElement('div');
      actionGroup.className = 'game-action-group';
      const quickBtn = document.createElement('button');
      quickBtn.className = 'game-cta game-quick-match-btn';
      quickBtn.type = 'button';
      quickBtn.dataset.quickMatchGame = game.name === 'Pişti' ? 'pisti' : 'chess';
      quickBtn.dataset.requiresAuth = 'true';
      quickBtn.dataset.gameName = game.name;
      ...
      quickBtn.addEventListener('click', async (event) => {
        ...
        if (typeof starter === 'function') {
          await starter(game.name === 'Pişti' ? 'pisti' : 'chess', game.name === 'Pişti' ? { mode: '2-52', bet: 1000 } : {});
        }
      }, true);
      actionGroup.append(quickBtn, button);
      foot.appendChild(actionGroup);
    } else {
