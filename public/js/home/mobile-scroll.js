...
function getScrollableTarget(node) {
  return node?.closest?.('.ps-modal-body, .modal-body, .modal-content, .sheet-content, .avatar-picker-scroll, .frame-picker-scroll, .ps-chat-stream, [data-scroll-container="true"]') || null;
}
...
export function syncHomeModalScrollLock() {
  const active = !!document.querySelector('.ps-modal.active, .ps-modal.is-open, .sheet-shell.is-open, [data-home-modal-open="true"]');
...
export function installMobileGesturePolicy() {
  ...
  document.addEventListener('touchmove', (event) => {
    const activeModal = document.querySelector('.ps-modal.active, .ps-modal.is-open, .sheet-shell.is-open, [data-home-modal-open="true"]');
