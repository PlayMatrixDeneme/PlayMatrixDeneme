...
function openHomeModalById(id) {
  if (window.__PM_HOME_MODAL_MANAGER__?.openHomeModal) return window.__PM_HOME_MODAL_MANAGER__.openHomeModal(id);
  const modal = byId(id);
...
function closeHomeModalById(id) {
  if (window.__PM_HOME_MODAL_MANAGER__?.closeHomeModal) return window.__PM_HOME_MODAL_MANAGER__.closeHomeModal(id);
  const modal = byId(id);
  if (!modal) return false;
...
function installPhase7ProfilePanelShortcuts() {
  if (document.body?.dataset.phase7ProfileBound === '1') return;
  if (document.body) document.body.dataset.phase7ProfileBound = '1';
  document.addEventListener('click', (event) => {
    const quickAvatar = event.target?.closest?.('#profileAvatarQuickBtn');
    if (quickAvatar && typeof window.showPlayerStats === 'function') {
      const uid = window.__PM_RUNTIME?.auth?.currentUser?.uid || '';
      if (uid) window.showPlayerStats(uid);
    }
    const wheelAction = event.target?.closest?.('[data-shell-action="wheel"]');
    if (wheelAction && typeof window.openSheet === 'function') {
      event.preventDefault();
      window.openSheet('wheel', 'Günlük Çark', 'Hazırsan günlük ödülünü çevir.');
    }
  }, true);
}
...
export function installProfilePanelGuards() {
  ...
  installPhase7ProfilePanelShortcuts();
}
