(function () {
  ...
  const VERSION = 'release-20260430';
  const ONLINE_GAME_RE = /\/Online\s*Oyunlar\/(Crash|Satranc|Pisti)(?:\.html)?$/i;
  ...
  function safeLocalAssetUrl(value, fallback) {
    const raw = txt(value, fallback || '');
    if (!raw) return resolveLocalPath(fallback || '');
    try {
      const parsed = new URL(raw, window.location.origin);
      if (!['http:', 'https:'].includes(parsed.protocol)) return resolveLocalPath(fallback || '');
      return parsed.href;
    } catch (_) {
      return resolveLocalPath(fallback || '');
    }
  }
  function createIconNode(className) {
    const icon = document.createElement('i');
    icon.className = className || 'fa-solid fa-xmark';
    icon.setAttribute('aria-hidden', 'true');
    return icon;
  }
  function createCloseButton(modalId) {
    ...
    button.type = 'button';
    button.dataset.pmAction = 'closeMatrixModal';
    button.dataset.pmArgs = JSON.stringify([modalId]);
    button.setAttribute('aria-label', 'Kapat');
    button.appendChild(createIconNode('fa-solid fa-xmark'));
    return button;
  }
  function buildPlayerStatsEmpty(title, bodyText) {
    const header = document.createElement('div');
    header.className = 'ps-modal-header';
    header.append(createText('div', 'ps-modal-title', title), createCloseButton('playerStatsModal'));
    const body = document.createElement('div');
    body.className = 'ps-modal-body';
    const empty = document.createElement('div');
    empty.className = 'ps-empty-state';
    empty.appendChild(createText('strong', '', bodyText));
    body.appendChild(empty);
    return [header, body];
  }
  function buildPickerEmpty(textValue) {
    return createText('div', 'avatar-picker-empty', textValue);
  }
  function buildSkeletonGameCard() {
    const card = document.createElement('article');
    card.className = 'game-card skeleton is-skeleton';
    const top = document.createElement('div');
    top.className = 'game-top';
    const icon = document.createElement('div');
    icon.className = 'game-icon skeleton';
    const tags = document.createElement('div');
    tags.className = 'tag-stack';
    tags.append(createText('span', 'mini-tag skeleton', ''), createText('span', 'mini-tag skeleton', ''));
    top.append(icon, tags);
    const body = document.createElement('div');
    body.className = 'game-body';
    body.append(createText('h3', 'game-title skeleton', ''), createText('div', 'game-desc skeleton', ''));
    const footer = document.createElement('div');
    footer.className = 'game-footer';
    footer.append(createText('span', 'game-status skeleton', ''), createText('span', 'game-cta skeleton', ''));
    card.append(top, body, footer);
    return card;
  }
  function buildAvatarImage(user) {
    const img = document.createElement('img');
    img.src = safeLocalAssetUrl(user?.avatar, FALLBACK_AVATAR);
    ...
    img.addEventListener('error', () => {
      ...
      img.src = resolveLocalPath(FALLBACK_AVATAR);
    }, { once: true });
    return img;
  }
  function buildPlayerStatsFallbackContent({ name, rank, metric, score }) {
    const header = document.createElement('div');
    header.className = 'ps-modal-header';
    header.append(createText('div', 'ps-modal-title', 'Oyuncu İstatistikleri'), createCloseButton('playerStatsModal'));
    const body = document.createElement('div');
    body.className = 'ps-modal-body player-stats-modal-body';
    const panel = document.createElement('section');
    panel.className = 'player-stats-panel';
    panel.appendChild(createText('h4', 'player-stats-panel-title', name));
    const rows = document.createElement('div');
    rows.className = 'player-stats-rows';
    [
      ['Sıra', rank],
      [metric, score],
      ['Durum', 'Genel görünüm']
    ...
    body.append(panel, createText('div', 'ps-modal-note', 'Detaylı profil istatistikleri giriş yapıldığında sunucudan çekilir.'));
    return [header, body];
  }
  ...
  const FALLBACK_GAMES = [
    { name: 'Crash', category: 'online', access: 'auth', url: '/online-games/crash', icon: 'fa-arrow-trend-up', desc: 'Refleks ve zamanlama odaklı online multiplier oyunu.', tags: ['Canlı Oyun', 'Rekabet', 'Hızlı Tur'] },
    { name: 'Satranç', category: 'online', access: 'auth', url: '/online-games/chess', icon: 'fa-chess', desc: 'Modern arayüzlü online satranç arenası.', tags: ['PvP', 'Strateji', 'Arena'] },
    { name: 'Pişti', category: 'online', access: 'auth', url: '/online-games/pisti', icon: 'fa-layer-group', desc: 'Gerçek oyuncularla online pişti masaları.', tags: ['Kart', 'Online', 'Klasik'] },
    { name: 'Pattern Master', category: 'classic', access: 'auth', url: '/classic-games/pattern-master', icon: 'fa-shapes', desc: 'Dikkat ve görsel hafıza oyunu.', tags: ['Ücretsiz', 'Zeka', 'Refleks'] },
    { name: 'Space Pro', category: 'classic', access: 'auth', url: '/classic-games/space-pro', icon: 'fa-user-astronaut', desc: 'Hızlı klasik arcade uzay oyunu.', tags: ['Arcade', 'Retro', 'Ücretsiz'] },
    { name: 'Snake Pro', category: 'classic', access: 'auth', url: '/classic-games/snake-pro', icon: 'fa-wave-square', desc: 'Retro hisli akıcı snake oyunu.', tags: ['Retro', 'Arcade', 'Ücretsiz'] }
  ];
  ...
  function ensureModalContent(id, title, body) {
    ...
    if (id === 'playerStatsModal') {
      content.replaceChildren(...buildPlayerStatsEmpty(title, body));
    }
  }
  function openAvatarFallback() {
    const container = $('avatarCategoryContainer');
    if (container && !container.children.length) {
      container.replaceChildren(buildPickerEmpty('Avatar kataloğu hazırlanıyor. Sayfa tamamen yüklendiğinde seçenekler otomatik görünür.'));
    }
  ...
  function openFrameFallback() {
    ...
    if (container && !container.children.length) {
      container.replaceChildren(buildPickerEmpty('frames kataloğu hazırlanıyor. Hesap seviyesi verisi gelince seçenekler otomatik görünür.'));
    }
  ...
  function installGlobalFallbacks() {
    window.renderGameShowcaseSkeleton = window.renderGameShowcaseSkeleton || function renderGameShowcaseSkeleton(count) {
      ...
      for (let i = 0; i < total; i += 1) {
        fragment.appendChild(buildSkeletonGameCard());
      }
      grid.replaceChildren(fragment);
    };
  ...
  function createLeaderboardItem(user, index, tab) {
    ...
    avatar.replaceChildren(buildAvatarImage(user));
  ...
  function renderPlayerStatsFallback(target) {
    ...
    content.replaceChildren(...buildPlayerStatsFallbackContent({ name, rank, metric, score }));
  ...
  function installClickGuards() {
    ...
    document.addEventListener('click', (event) => {
      ...
      const onlineTrigger = event.target.closest?.('a[href*="online-games"], [data-requires-auth="true"], .game-card');
      if (onlineTrigger) {
