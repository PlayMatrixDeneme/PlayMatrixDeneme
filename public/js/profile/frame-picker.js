...
export function createFramePicker({
  ...
  function buildFrameCard(level, selectedFrame, currentLevel, avatar) {
    ...
    card.setAttribute('aria-label', normalizedLevel === 0 ? 'framessiz profil görünümü' : `Seviye ${normalizedLevel} çerçevesi`);
    ...
    title.textContent = normalizedLevel === 0 ? 'framessiz' : `Seviye ${normalizedLevel}`;
    ...
    asset.textContent = frameAssetIndex > 0 ? `frames ${frameAssetIndex}` : 'Standart';
  ...
  function renderFrameOptions() {
    ...
    summaryTitle.textContent = selectedFrame > 0 ? `Seviye ${selectedFrame} framessi` : 'framessiz Görünüm';
