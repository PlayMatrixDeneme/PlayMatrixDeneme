...
const { APP_LOG_PATH, logCaughtError } = require('../utils/logger');
...
const { buildCanonicalUserState, normalizeSelectedFrameLevel, isSelectedFrameUnlocked, assertSelectedFrameSelectable } = require('../utils/accountState');
const { normalizeUserRankState, buildActivityResetState } = require('../utils/progression');
const { restrictionSnapshot } = require('../utils/userRestrictions');
const { DEFAULT_AVATAR, sanitizeAvatarForStorage, validateAvatarUrl } = require('../utils/avatarManifest');
const {
...
const FORBIDDEN_COSMETIC_PATCH_KEYS = new Set([
  'frame', 'frameUrl', 'frameSrc', 'frameImage', 'framePng', 'frameSvg', 'selectedFrameUrl',
  'activeFrame', 'activeFrameClass', 'activeFrameUrl', 'avatarSrc', 'customAvatar', 'customFrame'
]);

function normalizeAdminIdentityPatch(requestedPatch = {}, currentUser = {}) {
  const patch = requestedPatch && typeof requestedPatch === 'object' ? requestedPatch : {};
  for (const key of Object.keys(patch)) {
    if (FORBIDDEN_COSMETIC_PATCH_KEYS.has(key)) {
      const error = new Error(`${key} alanı güvenli kozmetik kontratına aykırı. Sadece kayıtlı avatar linki ve sayısal selectedFrame kabul edilir.`);
  ...
  if (Object.prototype.hasOwnProperty.call(patch, 'avatar')) {
    patch.avatar = validateAvatarUrl(patch.avatar, { storage: true });
  }

  if (Object.prototype.hasOwnProperty.call(patch, 'selectedFrame')) {
    const numericSelectedFrame = Number(patch.selectedFrame);
    if (!Number.isFinite(numericSelectedFrame) || numericSelectedFrame < 0) {
      const error = new Error('Geçersiz çerçeve seçimi. selectedFrame sadece 0-100 arası sayısal seviye olabilir.');
      error.statusCode = 400;
      throw error;
    }
    const safeSelectedFrame = normalizeSelectedFrameLevel(numericSelectedFrame);
    const mergedForFrame = { ...currentUser, ...patch };
    assertSelectedFrameSelectable(safeSelectedFrame, mergedForFrame);
    patch.selectedFrame = safeSelectedFrame;
  }
...
const AUDITED_USER_VALUE_FIELDS = Object.freeze(['balance', 'accountLevel', 'accountXp', 'accountLevelScore', 'selectedFrame']);
const AUDITED_USER_VALUE_FIELD_SET = new Set(AUDITED_USER_VALUE_FIELDS);

function normalizeAuditedUserValue(field = '', value = 0) {
  const safeField = cleanStr(field || '', 80);
  if (safeField === 'selectedFrame' || safeField === 'accountLevel') return Math.max(0, Math.floor(safeNum(value, 0)));
  return Math.floor(safeNum(value, 0));
}

function pickAuditedUserValues(user = {}) {
  const source = user && typeof user === 'object' ? user : {};
  const canonical = buildCanonicalUserState(source, { defaultFrame: 0 });
  return {
    balance: normalizeAuditedUserValue('balance', source.balance),
    accountLevel: normalizeAuditedUserValue('accountLevel', canonical.accountLevel),
    accountXp: normalizeAuditedUserValue('accountXp', canonical.accountXp),
    accountLevelScore: normalizeAuditedUserValue('accountLevelScore', canonical.accountLevelScore),
    selectedFrame: normalizeAuditedUserValue('selectedFrame', canonical.selectedFrame)
  };
}

function buildAuditedUserValueDiff(before = {}, after = {}, fields = AUDITED_USER_VALUE_FIELDS) {
  const allowedFields = (Array.isArray(fields) && fields.length ? fields : AUDITED_USER_VALUE_FIELDS)
    .map((field) => cleanStr(field || '', 80))
    .filter((field) => AUDITED_USER_VALUE_FIELD_SET.has(field));
  const prev = pickAuditedUserValues(before);
  const next = pickAuditedUserValues(after);
  const diff = {};
  for (const field of allowedFields) {
    if (prev[field] !== next[field]) diff[field] = { before: prev[field], after: next[field] };
  }
  return diff;
}

function buildAuditedUserValueMetadata(before = {}, after = {}, fields = AUDITED_USER_VALUE_FIELDS) {
  const diff = buildAuditedUserValueDiff(before, after, fields);
  return {
    auditedValueFields: Object.keys(diff),
    auditedValueDiff: diff
  };
}
...
async function recordAdminAudit(req, payload = {}) {
  return recordAdminOperationAudit(req, payload);
}
...
router.get('/admin/tickets', requireAdminPermission('tickets.read'), async (req, res) => {
  try {
    ...
    let query = colTickets().orderBy('updatedAt', 'desc').limit(limit);
    if (status) query = colTickets().where('status', '==', status).orderBy('updatedAt', 'desc').limit(limit);
    const snap = await query.get();
    const tickets = snap.docs.map((doc) => formatTicketSnapshot(doc));
    res.setHeader('Cache-Control', 'no-store, max-age=0');
    res.json({ ok: true, tickets, count: tickets.length });
  } catch (_error) {
...
router.post('/admin/tickets/:id/status', requireAdminPermission('tickets.write'), async (req, res) => {
  try {
    ...
    const note = sanitizeSupportText(req.body?.note || '', 1000);
    if (!ticketId) throw new Error('Geçersiz kayıt.');
    if (!status) throw new Error('Geçersiz durum.');
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (note) messages.push(buildSupportMessage({ id: `admin_${now}`, sender: 'admin', senderUid: req.user.uid, text: note, createdAt: now }));
    ...
    await Promise.all([
      recordAdminAudit(req, {
        ...
        targetId: ticketId,
        metadata: { status, noteAdded: !!note }
      }),
      recordSupportEvent({
        actorUid: req.user.uid,
        actorEmail: req.user.email || '',
        action: 'support.ticket.reply_admin',
        ticketId,
        metadata: { status, noteAdded: !!note }
      })
    ]);
    ...
    res.status(error.statusCode || 400).json({ ok: false, error: error.message || 'Durum güncellenemedi.' });
  }
});
...
router.patch('/admin/users/:uid', requireAdminPermission('users.write'), async (req, res) => {
  ...
  try {
    ...
    normalizeAdminIdentityPatch(requestedPatch, currentUser);
    const dangerousFields = assertCriticalPatchConfirmed(req, currentUser, requestedPatch);
    ...
    let emailSync = null;
    if (Object.prototype.hasOwnProperty.call(requestedPatch, 'email')) {
      emailSync = await validateAuthFirestoreEmailSync({ auth, uid, firestoreUser: { ...currentUser, ...patch } });
      if (!emailSync.ok) {
        const error = new Error('Firebase Auth ve Firestore e-posta senkronu doğrulanamadı.');
        error.statusCode = 409;
        throw error;
      }
    }
    ...
    await recordAdminAudit(req, {
      ...
      metadata: {
        ...
        ...buildAuditedUserValueMetadata(currentUser, { ...currentUser, ...patch }),
        authEmailSynced: !!emailRollbackState,
        emailSyncVerified: !!emailSync?.ok,
        previousAuthEmail: emailRollbackState?.email || '',
        dangerousFields
      }
    });
  ...
  try {
    ...
    await recordAdminAudit(req, {
      ...
      metadata: {
        ...
        ...buildAuditedUserValueMetadata(currentUser, { ...currentUser, ...patch }, requestedFields)
      }
    });
...
router.post('/admin/users/bulk-reset', requireAdminPermission('users.write'), async (req, res) => {
  try {
    requireDangerousActionConfirmation(req, BULK_RESET_CONFIRMATION_TEXT, { message: 'Toplu sıfırlama için yazılı onay gerekli.' });
    ...
    await recordAdminAudit(req, {
      ...
      metadata: {
        fields: requestedFields,
        auditedValueFields: requestedFields.filter((field) => AUDITED_USER_VALUE_FIELD_SET.has(field)),
        resetDefaults: Object.fromEntries(requestedFields.filter((field) => AUDITED_USER_VALUE_FIELD_SET.has(field)).map((field) => [field, REMOTE_DEFAULTS[field]])),
...
router.post('/admin/activity/reset', requireAdminPermission('system.read', 'users.write'), async (req, res) => {
  try {
    requireDangerousActionConfirmation(req, ACTIVITY_RESET_CONFIRMATION_TEXT, { message: 'Aktiflik sıfırlama için yazılı onay gerekli.' });
    ...
    const result = await runUserBatches(() => {
      const resetState = buildActivityResetState(activityLabel, { resetAt: now, resetActivityPass: resetMonthly });
      const patch = {
        ...resetState,
      ...
      if (!resetMonthly) delete patch.monthlyActiveScore;
      return patch;
    });
...
router.post('/admin/rewards/grant', requireAdminPermission('rewards.write'), async (req, res) => {
  try {
    ...
    requireDangerousActionConfirmation(req, DANGEROUS_CONFIRMATION_TEXT, { message: 'Manuel ödül için yazılı onay gerekli.' });
    ...
    await recordAdminAudit(req, {
      ...
      targetType: 'user',
      targetId: uid,
      metadata: { amount, reason, ledgerId: grant.id, duplicated: !!grant.duplicated, auditedValueFields: ['balance'] }
    });
...
router.get('/admin/audit', requireAdminPermission('admin.read'), async (req, res) => {
  try {
...
router.get('/admin/feature-flags', requireAdminPermission('system.read'), async (req, res) => {
  try {
...
router.get('/admin/ops/health', requireAdminPermission('system.read'), async (req, res) => {
  try {
...
router.get('/admin/platform/control', requireAdminPermission('system.read'), async (req, res) => {
  try {
...
router.get('/admin/matrix/dashboard', requireAdminPermission('admin.read', 'users.read'), async (req, res) => {
  try {
...
router.post('/admin/matrix/reset-nuclear', requireAdminPermission('users.write'), async (req, res) => {
  try {
    requireDangerousActionConfirmation(req, DANGEROUS_CONFIRMATION_TEXT, { message: 'Kritik yönetici işlemi için yazılı onay gerekli.' });
    const requestedFields = Array.isArray(req.body?.fields)
      ? req.body.fields.map((item) => cleanStr(item, 60)).filter((item) => Object.prototype.hasOwnProperty.call(REMOTE_DEFAULTS, item))
      : [];
...
router.post('/admin/matrix/restrict-user', requireAdminPermission('moderation.write'), async (req, res) => {
  try {
    ...
    requireDangerousActionConfirmation(req, DANGEROUS_CONFIRMATION_TEXT, { message: 'Kullanıcı kısıtlaması için yazılı onay gerekli.' });
    const reason = cleanStr(req.body?.reason || '', 500);
    ...
    await recordAdminAudit(req, { action: `matrix.restrict.${action}`, targetType: 'user', targetId: target.uid, metadata: { durationMinutes, reason } });
...
router.post('/admin/matrix/reward-user', requireAdminPermission('rewards.write'), async (req, res) => {
  try {
    const target = await resolveUserByIdentifier(req.body?.identifier || req.body?.uid || '');
    requireDangerousActionConfirmation(req, DANGEROUS_CONFIRMATION_TEXT, { message: 'Kullanıcı ödülü için yazılı onay gerekli.' });
    const amount = Math.max(1, Math.min(100000000, Math.floor(safeSignedNum(req.body?.amount, 0))));
    const reason = cleanStr(req.body?.reason || '', 240);
    const grant = await grantReward({
      uid: target.uid,
      amount,
      source: 'admin_manual_grant',
      referenceId: `admin_matrix:${req.user.uid}`,
      idempotencyKey: cleanStr(req.body?.idempotencyKey || `admin_matrix_user:${req.user.uid}:${target.uid}:${nowMs()}`, 220),
      actorUid: req.user.uid,
      reason
    });
    await recordAdminAudit(req, { action: 'matrix.reward_user', targetType: 'user', targetId: target.uid, metadata: { amount, reason, ledgerId: grant.id, duplicated: !!grant.duplicated, auditedValueFields: ['balance'] } });
...
router.post('/admin/matrix/reward-all', requireAdminPermission('rewards.write'), async (req, res) => {
  try {
    requireDangerousActionConfirmation(req, DANGEROUS_CONFIRMATION_TEXT, { message: 'Toplu ödül için yazılı onay gerekli.' });
    const amount = Math.max(1, Math.min(1000000, Math.floor(safeSignedNum(req.body?.amount, 0))));
    const reason = cleanStr(req.body?.reason || '', 240);
    const bulkId = cleanStr(req.body?.bulkId || `admin_reward_all:${req.user.uid}:${nowMs()}`, 180);
    ...
    await recordAdminAudit(req, { action: 'matrix.reward_all', targetType: 'system', targetId: 'all_users', metadata: { amount, reason, auditedValueFields: ['balance'], ...result } });
...
router.post('/admin/matrix/promo-codes', requireAdminPermission('rewards.write'), async (req, res) => {
  try {
    ...
    requireDangerousActionConfirmation(req, DANGEROUS_CONFIRMATION_TEXT, { message: 'Promosyon kodu oluşturmak için yazılı onay gerekli.' });
    ...
    await recordAdminAudit(req, { action: 'matrix.promo_create', targetType: 'promo', targetId: code, metadata: { amount, usageLimit, durationHours, onePerAccount } });
...
router.get('/admin/matrix/promos', requireAdminPermission('rewards.read'), async (req, res) => {
  try {
...
router.patch('/admin/matrix/maintenance', requireAdminPermission('system.read'), async (req, res) => {
  try {
    const current = await getFeatureFlagsDocument();
    const nextFlags = sanitizeFeatureFlags({
      ...current,
      crashMaintenance: req.body?.crash === true,
      pistiMaintenance: req.body?.pisti === true,
      chessMaintenance: req.body?.chess === true,
      classicGamesMaintenance: req.body?.classic === true
    }, DEFAULT_FEATURE_FLAGS);
    await setFeatureFlagsDocument(nextFlags, req.user?.uid || '');
    await recordAdminAudit(req, { action: 'matrix.maintenance_update', targetType: 'system', targetId: 'maintenance', metadata: { crash: nextFlags.crashMaintenance, pisti: nextFlags.pistiMaintenance, chess: nextFlags.chessMaintenance, classic: nextFlags.classicGamesMaintenance } });
    ...
    return res.status(400).json({ ok: false, error: error.message || 'maintenance modu güncellenemedi.' });
  }
});

router.get('/admin/matrix/issues', requireAdminPermission('system.read'), async (req, res) => {
  try {
