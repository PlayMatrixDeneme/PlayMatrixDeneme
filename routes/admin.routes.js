...
const { buildCanonicalUserState, normalizeSelectedFrameLevel, isSelectedFrameUnlocked } = require('../utils/accountState');
...
function normalizeAdminIdentityPatch(requestedPatch = {}, currentUser = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(patch, 'avatar')) {
    const normalizedAvatar = sanitizeAvatarForStorage(patch.avatar);
    if (!normalizedAvatar) {
      const error = new Error('Geçersiz avatar seçimi. Sadece avatar manifest içinde kayıtlı linkler kullanılabilir.');
      error.statusCode = 400;
      throw error;
    }
    patch.avatar = normalizedAvatar;
  }
  ...
  if (Object.prototype.hasOwnProperty.call(patch, 'selectedFrame')) {
    ...
    const mergedForLevel = { ...currentUser, ...patch };
    const accountLevel = buildCanonicalUserState(mergedForLevel, { defaultFrame: 0 }).accountLevel;
    if (!isSelectedFrameUnlocked(safeSelectedFrame, accountLevel)) {
      const error = new Error(`Bu çerçeveyi kaydetmek için kullanıcı en az Seviye ${safeSelectedFrame} olmalı.`);
      error.statusCode = 400;
      throw error;
    }
