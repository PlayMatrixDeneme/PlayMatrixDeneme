...
router.get('/auth/admin/matrix/status', async (req, res) => {
  try {
    ...
    const clientKey = cleanStr(req.headers['x-admin-client-key'] || '', 2000);
    ...
    const expectedUid = cleanStr(context.uid || '', 160);
    const expectedEmail = normalizeEmail(context.email || '');
    const clientKeyMatches = !!(clientKeyState.ok
      && clientKeyState.payload.uid === expectedUid
      && normalizeEmail(clientKeyState.payload.email || '') === expectedEmail
      && currentSessionId
      && clientKeyState.payload.sessionId === currentSessionId);
    ...
    if (!clientKeyMatches) {
      return res.status(403).json({
        ...
        authenticated: true,
        redirectTo: '/admin/index.html',
        code: clientKeyState.code || 'ADMIN_CLIENT_KEY_REQUIRED',
    ...
    return res.json({
      ...
      admin: serializeAdminContext(context),
      clientKey: issueClientGateKey({ uid: context.uid, email: context.email, sessionId: currentSessionId })
    });
  } catch (_error) {
  ...
  try {
    ...
    if (adminContext?.isAdmin) {
      ...
      securityComplete = !!(req.user?.sessionSource === 'admin_matrix'
        && clientKeyState.ok
        && clientKeyState.payload.uid === user.uid
        && normalizeEmail(clientKeyState.payload.email || '') === user.email
        && cleanStr(clientKeyState.payload.sessionId || '', 160) === cleanStr(req.user?.sessionId || '', 160));
    }
