...
function getChessLastPing(room = {}) {
  return Math.max(0, safeNum(room.host?.lastPing, 0), safeNum(room.guest?.lastPing, 0), safeNum(room.updatedAt, 0), safeNum(room.createdAt, 0));
}

function isChessLobbyVisible(room = {}, now = nowMs()) {
  const status = cleanStr(room.status || '', 24);
  if (!['waiting', 'playing'].includes(status)) return false;
  if (status === 'waiting' && getChessLastPing(room) && now - getChessLastPing(room) > CHESS_DISCONNECT_GRACE_MS * 2) return false;
...
router.get('/lobby', verifyAuth, async (_req, res) => {
  ...
  try {
    ...
    snapWait.forEach((doc) => {
      const d = doc.data() || {};
      if (!isChessLobbyVisible(d, now)) { doc.ref.delete().catch(() => null); return; }
      rooms.push({ id: doc.id, hostUid: d.host?.uid || '', host: d.host?.username || 'Oyuncu', guest: null, status: d.status, createdAt: safeNum(d.createdAt, 0), updatedAt: safeNum(d.updatedAt, 0) });
    });
    snapPlay.forEach((doc) => {
      const d = doc.data() || {};
      if (!isChessLobbyVisible(d, now)) return;
      rooms.push({ id: doc.id, hostUid: d.host?.uid || '', host: d.host?.username || 'Oyuncu', guest: d.guest ? d.guest.username : 'Bilinmeyen', status: d.status, createdAt: safeNum(d.createdAt, 0), updatedAt: safeNum(d.updatedAt, 0) });
    });
  ...
  try {
    ...
    const roomData = await db.runTransaction(async (tx) => {
      ...
      if (roomId) {
        ...
        if (cleanStr(r.status || '', 24) === 'waiting' && !isChessLobbyVisible(r, now)) {
          tx.delete(roomRef);
          throw new Error('Bu oda artık aktif değil. Lütfen yeni oda kurun.');
        }
      ...
      snap.forEach((doc) => {
        const data = doc.data() || {};
        if (!isChessLobbyVisible(data, now)) { tx.delete(doc.ref); return; }
        if (cleanStr(data?.host?.uid || '', 160) !== uid && !docToJoin) docToJoin = doc;
      });
