...
router.post('/online/play-open', verifyAuth, async (req, res) => {
  try {
    ...
    const roomData = await db.runTransaction(async (tx) => {
      ...
      const newRoom = { mode, maxPlayers, deckCount, bet, isPrivate: false, roomName: 'Açık Masa', password: '', players: [{ uid, username: uSnap.data()?.username || 'Oyuncu', avatar: sanitizePublicAvatarForOutput(uSnap.data()?.avatar), selectedFrame: pickUserSelectedFrame(uSnap.data() || {}), score: 0, cardCount: 0, hand: [], lastPing: nowMs(), processingUntil: 0 }], participantUids: [uid], tableCards: [], deck: [], turn: 0, status: 'waiting', finishReason: '', cleanupAt: 0, resumeAvailableUntil: 0, createdAt: nowMs(), updatedAt: nowMs(), stateVersion: 1, settlementStatus: GAME_SETTLEMENT_STATUS.ACTIVE, resultCode: '', resultReason: '', settledAt: 0, payoutByUid: {}, timeline: [buildTimelineEvent('room_created', { actorUid: uid, roomId, gameKey: 'pisti', status: 'waiting', participantUids: [uid], meta: { mode, bet } })] };
