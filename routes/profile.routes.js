...
const colPromoClaims = () => db.collection('promo_claims');
...
router.get('/', verifyAuth, async (req, res) => {
  try {
    ...
    const safeUser = {
      ...
      emailVerified: !!req.user.email_verified,
      createdAt: getFirestoreTimestampMs(uData.u.createdAt, nowMs()),
      memberSince: getFirestoreTimestampMs(uData.u.createdAt, nowMs()),
...
router.get('/me/active-sessions', verifyAuth, async (req, res) => {
  try {
    ...
    const items = sessions.map((item) => ({
      ...item,
      resumePath: item.gameType === 'pisti' ? '/online-games/pisti' : '/online-games/chess',
...
router.post('/update', verifyAuth, profileLimiter, async (req, res) => {
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (avatar !== undefined) {
        const normalizedAvatar = validateAvatarUrl(avatar, { storage: true });
    ...
    res.json({
      ok: true,
      selectedFrame: freshCanonical.selectedFrame,
      frameState: buildFrameState(freshCanonical.selectedFrame, freshCanonical.accountLevel),
      accountLevel: freshCanonical.accountLevel,
      accountXp: freshCanonical.accountXp,
      progression: freshCanonical.progression
    });
  } catch (e) {
...
router.post('/bonus/claim', verifyAuth, bonusLimiter, async (req, res) => {
  try {
    ...
    const out = await db.runTransaction(async (tx) => {
      ...
      const promoGuardId = `promo_code:${code}:${req.user.uid}`;
      const canonicalPromoGuardId = `promo_code:${promoDocId}:${req.user.uid}`;
      const promoGuardRefs = Array.from(new Map([promoGuardId, canonicalPromoGuardId].map((id) => [id, colPromoClaims().doc(id)])).values());

      const [uSnap, pSnap, ...claimSnaps] = await Promise.all([tx.get(uRef), tx.get(pRef), ...promoGuardRefs.map((ref) => tx.get(ref))]);
      if (!uSnap.exists || !pSnap.exists) throw new Error("Geçersiz işlem.");

      const u = uSnap.data() || {}, p = pSnap.data() || {};
      const onePerAccount = p.onePerAccount !== false;
      const claimAt = nowMs();
      if (safeNum(p.amount, 0) <= 0) throw new Error("Kod geçersiz veya kullanılmış.");
      if (p.active === false) throw new Error("Kod pasif durumda.");
      if (safeNum(p.expiresAt, 0) > 0 && safeNum(p.expiresAt, 0) < claimAt) throw new Error("Kod süresi dolmuş.");

      const usedPromos = Array.isArray(u.usedPromos) ? u.usedPromos.map((item) => cleanStr(item || '').toUpperCase()).filter(Boolean) : [];
      if (onePerAccount && (claimSnaps.some((snap) => snap.exists) || usedPromos.includes(code) || usedPromos.includes(promoDocId))) throw new Error("Kod geçersiz veya kullanılmış.");
      if (safeNum(p.limitLeft, -1) === 0) throw new Error("Kod tükenmiş.");

      const promoClaimId = onePerAccount ? promoGuardId : `promo_code:${promoDocId}:${req.user.uid}:${claimAt}`;
      const claimRefs = onePerAccount ? promoGuardRefs : [colPromoClaims().doc(promoClaimId)];
      const grant = await applyRewardGrantInTransaction(tx, {
        uid: req.user.uid,
        amount: p.amount,
        source: 'promo_code',
        referenceId: promoDocId,
        idempotencyKey: onePerAccount ? `promo_code:${promoDocId}:${req.user.uid}` : `promo_code:${promoDocId}:${req.user.uid}:${claimAt}`,
        meta: { code, promoDocId, promoClaimId, onePerAccount, guardIds: claimRefs.map((ref) => ref.id) },
        userPatch: { usedPromos: admin.firestore.FieldValue.arrayUnion(code, promoDocId) }
      });
      const claimPayload = {
        uid: req.user.uid,
        code,
      ...
      claimRefs.forEach((claimRef) => tx.set(claimRef, { ...claimPayload, promoClaimId: claimRef.id }, { merge: false }));
      if (safeNum(p.limitLeft, -1) > 0 && !grant.duplicated) tx.update(pRef, { limitLeft: admin.firestore.FieldValue.increment(-1), lastClaimedAt: claimAt });
