...
router.post('/bonus/claim', verifyAuth, bonusLimiter, async (req, res) => {
  try {
    ...
    const out = await db.runTransaction(async (tx) => {
      ...
      const singleUseClaimRef = colPromoClaims().doc(`_`);

      const [uSnap, pSnap, claimSnap] = await Promise.all([tx.get(uRef), tx.get(pRef), tx.get(singleUseClaimRef)]);
      ...
      if (onePerAccount && (claimSnap.exists || (u.usedPromos || []).includes(code))) throw new Error("Kod geçersiz veya kullanılmış.");
      if (safeNum(p.limitLeft, -1) === 0) throw new Error("Kod tükenmiş.");

      const promoClaimId = onePerAccount ? `${promoDocId}_${req.user.uid}` : `${promoDocId}_${req.user.uid}_${claimAt}`;
      const claimRef = onePerAccount ? singleUseClaimRef : colPromoClaims().doc(promoClaimId);
      const grant = await applyRewardGrantInTransaction(tx, {
        uid: req.user.uid,
        amount: p.amount,
        source: 'promo_code',
        ...
        meta: { code, promoDocId, promoClaimId, onePerAccount },
      ...
      if (safeNum(p.limitLeft, -1) > 0) tx.update(pRef, { limitLeft: admin.firestore.FieldValue.increment(-1), lastClaimedAt: claimAt });
