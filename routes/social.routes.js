...
router.post('/friends/request', verifyAuth, profileLimiter, async (req, res) => {
  try {
    ...
    if (outcome.acceptedNow) {
      const payload = { uid, friendUid: target.uid, friendshipId, accepted: true, ts: nowMs() };
      emitSocialEvent(io, [uid, target.uid], SOCIAL_EVENT_TYPES.FRIEND_REQUEST_ACCEPTED, payload, {
        actorUid: uid,
        targetUid: target.uid,
        eventId: `friend:${friendshipId}:accepted:auto`,
        legacyEvents: [
          { name: 'friends:updated', payload: { ts: payload.ts } },
          { name: 'friends:request_auto_accepted', payload }
        ]
      });
      return res.json({ ok: true, acceptedNow: true, message: 'Karşılıklı istek bulundu ve arkadaşlık otomatik onaylandı.' });
    }

    const payload = { fromUid: uid, targetUid: target.uid, friendshipId, ts: nowMs() };
    emitSocialEvent(io, [target.uid], SOCIAL_EVENT_TYPES.FRIEND_REQUEST_CREATED, payload, {
      actorUid: uid,
      targetUid: target.uid,
      eventId: `friend:${friendshipId}:created`,
      legacyEvents: [{ name: 'friends:request_received', payload }]
    });
    emitSocialEvent(io, [uid], SOCIAL_EVENT_TYPES.FRIEND_REQUEST_CREATED, payload, {
      actorUid: uid,
      targetUid: target.uid,
      eventId: `friend:${friendshipId}:created:sender`,
      legacyEvents: [{ name: 'friends:updated', payload: { ts: payload.ts } }]
    });
...
router.post('/friends/respond', verifyAuth, profileLimiter, async (req, res) => {
  try {
    ...
    const io = req.app.get('io');
    const payload = { uid: related.recipientUid, requesterUid: related.requesterUid, recipientUid: related.recipientUid, accepted: related.accepted, ts: nowMs() };
    emitSocialEvent(io, [related.requesterUid, related.recipientUid], related.accepted ? SOCIAL_EVENT_TYPES.FRIEND_REQUEST_ACCEPTED : SOCIAL_EVENT_TYPES.FRIEND_REQUEST_DECLINED, payload, {
      actorUid: uid,
      targetUid: related.requesterUid,
      eventId: `friend:${friendshipId}:${related.accepted ? 'accepted' : 'declined'}`,
      legacyEvents: [
        { name: 'friends:request_result', payload },
        { name: 'friends:updated', payload: { ts: payload.ts } }
      ]
    });
...
router.post('/friends/remove', verifyAuth, profileLimiter, async (req, res) => {
  try {
    ...
    const io = req.app.get('io');
    const payload = { friendshipId, affected, removedBy: uid, ts: nowMs() };
    emitSocialEvent(io, affected, SOCIAL_EVENT_TYPES.FRIEND_REMOVED, payload, {
      actorUid: uid,
      eventId: `friend:${friendshipId}:removed`,
      legacyEvents: [{ name: 'friends:updated', payload: { ts: payload.ts } }]
    });
