...
const { upsertDirectMessageSearchIndex } = require('../utils/chatSearchIndex');
...
function getGamePath(gameType = '') {
  return normalizeGameType(gameType) === 'pisti' ? '/online-games/pisti' : '/online-games/chess';
}
...
function getPistiModeConfig(mode = '2-52') {
  const safeMode = cleanStr(mode || '2-52', 16);
  if (safeMode === '2-104') return { mode: '2-104', maxPlayers: 2, deckCount: 2 };
  if (safeMode === '4-104') return { mode: '4-104', maxPlayers: 4, deckCount: 2 };
  return { mode: '2-52', maxPlayers: 2, deckCount: 1 };
}
...
function createMultiPistiDeck(deckCount = 1) {
  ...
  const deck = [];
  for (let d = 0; d < Math.max(1, Math.floor(safeNum(deckCount, 1))); d += 1) {
    for (const s of suits) {
      for (const v of vals) deck.push(`${v + s}|${crypto.randomBytes(4).toString('hex')}`);
    }
  }
...
function buildPistiMatchPlayer(uid, userData, username) {
  return {
    uid: cleanStr(uid || '', 160),
    username: cleanStr(username || userData?.username || 'Oyuncu', 80),
    avatar: sanitizePublicAvatarForOutput(userData?.avatar),
...
function startPistiMatchmakingRoom(room) {
  room.status = 'playing';
  room.finishReason = '';
  room.winner = [];
  room.settlementStatus = GAME_SETTLEMENT_STATUS.ACTIVE;
  room.resultCode = '';
  room.resultReason = '';
  room.settledAt = 0;
  room.startedAt = nowMs();
  room.deck = createMultiPistiDeck(room.deckCount);
  room.tableCards = [room.deck.pop(), room.deck.pop(), room.deck.pop(), room.deck.pop()].filter(Boolean);
  for (const player of room.players || []) {
    player.hand = [room.deck.pop(), room.deck.pop(), room.deck.pop(), room.deck.pop()].filter(Boolean);
...
function recordPistiMatchmakingStake(tx, { uid = '', roomId = '', bet = 0, mode = '' } = {}) {
  const safeUid = cleanStr(uid || '', 160);
  const safeRoomId = cleanStr(roomId || '', 160);
  ...
  return recordGameLedgerInTransaction(tx, {
    uid: safeUid,
    gameType: 'pisti',
    source: 'pisti_matchmaking_stake',
    ...
    actorUid: safeUid,
    subjectUid: safeUid,
    meta: { action: 'quick_match', mode: cleanStr(mode || '', 16) },
    idempotencyKey: `pisti:${safeRoomId}:${safeUid}:matchmaking_stake`
  });
}
...
function getPresenceActivity(status = 'IDLE', gameType = '') {
  if (status === 'IN_GAME') {
    ...
    if (game === 'pisti') return 'Pişti Oynuyor';
    if (game === 'crash') return 'Crash Oynuyor';
    return 'Satranç Oynuyor';
  }
...
function emitPresenceUpdate(io, uid, source = 'socket') {
  ...
  io.emit('social:presence_update', payload);
  emitSocialEvent(io, [], SOCIAL_EVENT_TYPES.PRESENCE_UPDATED, payload, { actorUid: uid, eventId: `presence:${uid}:${payload.updatedAt}` });
}
...
function normalizeLobbyMessage(message = {}) {
  ...
  return {
    ...
    avatar: sanitizePublicAvatarForOutput(message.avatar),
...
async function persistLobbyChatMessage(payload) {
  try {
    await colLobbyChat().doc(payload.id).set({
      ...
      avatar: sanitizePublicAvatarForOutput(payload.avatar),
...
function formatDirectMessagePayload(message = {}, meta = {}) {
  ...
  return {
    ...
    avatar: sanitizePublicAvatarForOutput(message.avatar || meta.avatar),
...
async function loadDirectChatHistory(uid, targetUid) {
  ...
  const metaByUid = {
    ...
    [targetUid]: {
      ...
      avatar: sanitizePublicAvatarForOutput(targetMeta.avatar),
  ...
  return messagesSnap.docs.map((doc) => {
    ...
    return formatDirectMessagePayload({
      ...
      avatar: sanitizePublicAvatarForOutput(peerMeta.avatar),
...
async function createMatchmakingRoom(gameType, uidA, uidB, options = {}) {
  ...
  return db.runTransaction(async (tx) => {
    // Firestore transaction contract: all reads first, all writes after.
    ...
    if (safeGameType === 'pisti') {
      const { mode, maxPlayers, deckCount } = getPistiModeConfig(options.mode || '2-52');
      const bet = Math.max(1, Math.floor(safeNum(options.bet, 1000)));
      if (safeNum(a.balance, 0) < bet || safeNum(b.balance, 0) < bet) throw new Error('Pişti hızlı eşleşme için bakiye yetersiz.');
      const roomRef = colOnlinePisti().doc();
      const roomId = roomRef.id;
      const createdAt = nowMs();
      const players = [
        buildPistiMatchPlayer(uidA, a, aUsername),
        buildPistiMatchPlayer(uidB, b, bUsername)
      ];
      const room = startPistiMatchmakingRoom({
        mode,
        maxPlayers,
        deckCount,
        bet,
        ...
        timeline: [
          {
            ...
            participantUids: [uidA, uidB],
            createdAt,
            meta: { mode, bet, source: 'quick_match' }
          }
        ]
      });
      tx.update(aRef, { balance: admin.firestore.FieldValue.increment(-bet) });
      tx.update(bRef, { balance: admin.firestore.FieldValue.increment(-bet) });
      recordPistiMatchmakingStake(tx, { uid: uidA, roomId, bet, mode });
      recordPistiMatchmakingStake(tx, { uid: uidB, roomId, bet, mode });
      tx.set(roomRef, room);
      return { roomId, gameType: 'pisti', gamePath: '/online-games/pisti', mode, bet };
    }

    const roomRef = colChess().doc();
    tx.set(roomRef, {
      host: { uid: uidA, username: aUsername, avatar: sanitizePublicAvatarForOutput(a.avatar), selectedFrame: pickUserSelectedFrame(a), lastPing: nowMs() },
      guest: { uid: uidB, username: bUsername, avatar: sanitizePublicAvatarForOutput(b.avatar), selectedFrame: pickUserSelectedFrame(b), lastPing: nowMs() },
      status: 'playing',
      bet: 0,
    ...
    return { roomId: roomRef.id, gameType: 'chess', gamePath: '/online-games/chess' };
  });
}
...
async function syncPendingInvitesForSocket(socket, io, uid) {
  ...
  pendingInvites.forEach((invite) => {
    ...
    const receivePayload = {
      ...
      hostAvatar: sanitizePublicAvatarForOutput(invite.hostAvatar),
      hostSelectedFrame: invite.hostSelectedFrame,
      guestName: invite.guestName,
      guestAvatar: sanitizePublicAvatarForOutput(invite.guestAvatar),
    ...
    socket.emit('game:invite_receive', receivePayload);
    socket.emit('social:event', {
      eventId: `invite:${inviteId}:receive`,
      type: SOCIAL_EVENT_TYPES.GAME_INVITE_CREATED,
      schemaVersion: 1,
      createdAt: safeNum(invite.createdAt, nowMs()),
      actorUid: cleanStr(invite.hostUid || '', 160),
      targetUid: uid,
...
module.exports = function initSockets(io, auth) {
  ...
  io.on('connection', async (socket) => {
    ...
    if (!userPresence[uid] || getUserPresence(uid).status === 'OFFLINE' || userPresence[uid]?.disconnectTimer) syncSocketPresence('IDLE', 'Lobide', { reconnect: true });
    ...
    socket.on('social:set_presence', (data = {}) => {
      const rawStatus = cleanStr(data?.status || 'IDLE', 24).toUpperCase().replace(/[-\s]/g, '_');
      let nextStatus = rawStatus;
      if (['ONLINE', 'VISIBLE', 'ACTIVE'].includes(rawStatus)) nextStatus = 'IDLE';
      if (['INGAME', 'PLAYING', 'PLAYING_GAME', 'IN_GAME'].includes(rawStatus)) nextStatus = 'IN_GAME';
      if (!['IDLE', 'MATCHMAKING', 'IN_GAME', 'OFFLINE'].includes(nextStatus)) nextStatus = 'IDLE';
      const activity = cleanStr(data?.activity || '', 80);
      syncSocketPresence(nextStatus, activity, { gameType: data?.gameType || '' });
      touchSocketConnection({ socketId: socket.id, uid, status: nextStatus, activity, gameType: data?.gameType || '' }).catch(() => null);
    ...
    socket.on('chat:lobby_send', async (payload) => {
      try {
        ...
        const chatPayload = {
          ...
          avatar: sanitizePublicAvatarForOutput(u.avatar),
        ...
        emitSocialEvent(io, [], SOCIAL_EVENT_TYPES.CHAT_GLOBAL_CREATED, chatPayload, { actorUid: uid, eventId: `global:${chatPayload.id}` });
      } catch (_) {}
    });
    ...
    socket.on('chat:dm_send', async (payload) => {
      try {
        ...
        await upsertDirectMessageSearchIndex({ chatId, messageId, participants: [uid, targetUid], sender: uid, text: msg, createdAt, updatedAt: createdAt }).catch(() => null);
        ...
        emitSocialEvent(io, [uid, targetUid], SOCIAL_EVENT_TYPES.CHAT_DM_CREATED, dmPayload, { actorUid: uid, targetUid, eventId: `dm:${chatId}:${messageId}`, legacyEvents: [] });
    ...
    socket.on('game:invite_send', async (payload) => {
      try {
        ...
        emitSocialEvent(io, [targetUid], SOCIAL_EVENT_TYPES.GAME_INVITE_CREATED, inviteReceivePayload, {
          actorUid: uid,
          targetUid,
          eventId: `invite:${invite.inviteId}:created`,
          legacyEvents: [{ name: 'game:invite_receive', payload: inviteReceivePayload }]
        });
    ...
    socket.on('game:invite_response', async (payload) => {
      try {
        ...
        const basePayload = {
          ...
          guestUid: uid,
          guestName,
          guestAvatar: sanitizePublicAvatarForOutput(guest.avatar || finalizedInvite.guestAvatar),
        ...
        emitSocialEvent(io, [finalizedInvite.hostUid, uid], response === 'accepted' ? SOCIAL_EVENT_TYPES.GAME_INVITE_ACCEPTED : SOCIAL_EVENT_TYPES.GAME_INVITE_DECLINED, basePayload, {
          actorUid: uid,
          targetUid: finalizedInvite.hostUid,
          eventId: `invite:${inviteId}:${response}`,
          legacyEvents: [{ name: 'game:invite_response', payload: basePayload }]
        });
        socket.emit('game:invite_response', basePayload);
      } catch (error) {
    ...
    socket.on('game:matchmake_join', async (data) => {
      try {
        ...
        const reqData = {
          ...
          gameType,
          joinedAt: Date.now(),
          mode: gameType === 'pisti' ? cleanStr(data?.mode || '2-52', 16) : '',
          bet: gameType === 'pisti' ? Math.max(1, Math.floor(safeNum(data?.bet, 1000))) : 0
        };

        syncSocketPresence('MATCHMAKING', 'Eşleşme Aranıyor...', { gameType, mode: reqData.mode, bet: reqData.bet });
