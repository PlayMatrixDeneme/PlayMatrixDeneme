...
function validateStaticExample(env, failures, warnings) {
  ...
  if (String(env.SECURITY_CSP_STRICT || '') === '0' && String(env.SECURITY_CSP_REPORT_ONLY || '') !== '1') {
    warnings.push('SECURITY_CSP_STRICT=0 ise Faz 2 geçişinde SECURITY_CSP_REPORT_ONLY=1 olmalı.');
  }
  if (String(env.LEGACY_SESSION_HEADER_ENABLED || '0') !== '0') {
    failures.push('LEGACY_SESSION_HEADER_ENABLED production contract içinde 0 olmalı; session taşıma HttpOnly cookie merkezlidir.');
  }
}
