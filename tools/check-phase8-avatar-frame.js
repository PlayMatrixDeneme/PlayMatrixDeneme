...
[
  'sanitizeAvatarForStorage(avatar)',
...
[
  ...
  'sanitizeAvatarForStorage(patch.avatar)',
  'isSelectedFrameUnlocked(safeSelectedFrame, accountLevel)'
...
for (let index = 1; index <= 18; index += 1) {
  if (!exists(`Cerceve/frame-${index}.png`)) fail(`Cerceve/frame-${index}.png eksik.`);
}
const firstForbiddenFrame = `Cerceve/frame-${18 + 1}.png`;
if (exists(firstForbiddenFrame)) fail(`frame-${18 + 1} geri gelmiş; Faz 3 kararına aykırı.`);

['index.html', 'Online Oyunlar/Crash.html', 'Online Oyunlar/Pisti.html', 'Online Oyunlar/Satranc.html'].forEach((rel) => {
  const html = read(rel);
