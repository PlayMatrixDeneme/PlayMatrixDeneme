...
function mustNotContain(file, needle, label) {
  const text = read(file);
  if (text.includes(needle)) failures.push(`${label} kaldırılmalı: ${file}`);
}
...
mustContain('config/firebase.js', 'parseFirebaseKey', 'Firebase FIREBASE_KEY parser');
mustContain('config/firebase.js', 'firebase_admin_startup_failed', 'Firebase hard-fail log event');
mustContain('config/firebase.js', 'degraded: false', 'Firebase degraded=false status');
...
console.log('Runtime hardening kontrolü başarılı. Env, CORS, Firebase Admin hard-fail, public runtime, log redaction ve requestId standardı doğrulandı.');
process.exit(0);
