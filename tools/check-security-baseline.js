...
files.forEach((file) => {
  const relative = rel(file);
  if (relative === 'tools/check-security-baseline.js') return;
...
mustContain('.env.example', 'FIREBASE_KEY=', '.env.example Firebase service-account alanı');
...
mustContain('utils/env.js', 'FIREBASE_KEY raw service-account JSON', 'Firebase FIREBASE_KEY üretim standardı');
mustContain('utils/firebaseCredential.js', 'parseFirebaseKey', 'Firebase FIREBASE_KEY parser');
...
if (warnings.length) {
  console.warn('Security baseline uyarıları:');
  warnings.forEach((item) => console.warn(`- ${item}`));
}

if (failures.length) {
  console.error('Security baseline kontrolü başarısız:');
  failures.forEach((item) => console.error(`- ${item}`));
  process.exit(1);
}

console.log('Security baseline kontrolü başarılı. FIREBASE_KEY standardı, secret dosya koruması, hardcoded runtime fallback ve Firebase memory fallback kapalı.');
process.exit(0);
