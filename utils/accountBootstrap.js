...
const { recordRewardLedger, buildLedgerDocId } = require('./rewardLedger');
const { createNotification } = require('./notifications');

const colUsers = () => db.collection('users');
const colRewardLedger = () => db.collection('reward_ledger');

function buildBootstrapLedgerPayload({ uid = '', source = '', amount = 0, referenceId = 'account_bootstrap', idempotencyKey = '' } = {}) {
  ...
  const safeReferenceId = cleanStr(referenceId || 'account_bootstrap', 180);
  ...
  return {
    ledgerId,
    ledgerRef: colRewardLedger().doc(ledgerId),
    payload: {
      uid: safeUid,
      amount: safeAmount,
      currency: 'MC',
      source: safeSource,
      originalSource: safeSource,
      referenceId: safeReferenceId,
      meta: { accountBootstrap: true },
      idempotencyKey: safeIdempotencyKey,
      policyVersion: 1,
      status: 'committed',
      createdAt: nowMs(),
...
async function bootstrapAccountByAuth({ tx = null, uid = '', email = '', emailVerified = false, referenceId = 'account_bootstrap' } = {}) {
  ...
  const bootstrap = await runner(async (trx) => {
    ...
    const signupRewardAmount = getRewardAmount('signup_reward', 50000);
    const emailRewardAmount = getRewardAmount('email_verify_reward', 100000);
    const signupLedger = buildBootstrapLedgerPayload({
      uid: safeUid,
      amount: signupRewardAmount,
      source: 'signup_reward',
      referenceId,
    ...
    const emailLedger = buildBootstrapLedgerPayload({
      uid: safeUid,
      amount: emailRewardAmount,
      source: 'email_verify_reward',
      referenceId,
    ...
    const [signupLedgerSnap, emailLedgerSnap] = await Promise.all([
      trx.get(signupLedger.ledgerRef),
      trx.get(emailLedger.ledgerRef)
    ]);

    if (!user.signupRewardClaimed) {
      if (!signupLedgerSnap.exists) {
        user.balance = safeNum(user.balance, 0) + signupRewardAmount;
        trx.set(signupLedger.ledgerRef, signupLedger.payload, { merge: false });
        grantedSignupReward = true;
      }
      user.signupRewardClaimed = true;
      user.signupRewardLedgerRecorded = true;
