...
function normalizeOwnedFrameLevels(value = []) {
  const raw = Array.isArray(value)
    ? value
    : (typeof value === 'string' ? value.split(',') : []);
  return Array.from(new Set(raw
    .map((item) => normalizeSelectedFrameLevel(item))
    .filter((item) => item > 0 && item <= FRAME_LEVEL_CAP)))
    .sort((a, b) => a - b);
}

function isSelectedFrameOwned(selectedFrame = 0, user = {}) {
  const frameLevel = normalizeSelectedFrameLevel(selectedFrame);
  if (frameLevel <= 0) return true;
  return normalizeOwnedFrameLevels(user.ownedFrames || user.unlockedFrames || []).includes(frameLevel);
}

function isSelectedFrameSelectable(selectedFrame = 0, user = {}) {
  const frameLevel = normalizeSelectedFrameLevel(selectedFrame);
  if (frameLevel <= 0) return true;
  const accountLevel = normalizeAccountLevel(user.accountLevel ?? user.level ?? getAccountLevel(user));
  return isSelectedFrameUnlocked(frameLevel, accountLevel) || isSelectedFrameOwned(frameLevel, user);
}

function assertSelectedFrameSelectable(selectedFrame = 0, user = {}) {
  const frameLevel = normalizeSelectedFrameLevel(selectedFrame);
  if (frameLevel <= 0) return 0;
  if (isSelectedFrameSelectable(frameLevel, user)) return frameLevel;
  const error = new Error(`Bu çerçeveyi kullanmak için Seviye ${frameLevel} veya market sahipliği gerekir.`);
  error.code = 'FRAME_NOT_SELECTABLE';
...
function buildFrameState(selectedFrame = 0, accountLevel = 1, options = {}) {
  ...
  const userLike = options.user && typeof options.user === 'object' ? { ...options.user, accountLevel: safeAccountLevel } : { accountLevel: safeAccountLevel, ownedFrames: options.ownedFrames || [] };
  const levelUnlocked = isSelectedFrameUnlocked(safeSelectedFrame, safeAccountLevel);
  const owned = isSelectedFrameOwned(safeSelectedFrame, userLike);
  const unlocked = levelUnlocked || owned;
  ...
  return Object.freeze({
    ...
    levelUnlocked,
    owned,
    locked: safeSelectedFrame > 0 && !unlocked,
...
function buildCanonicalUserState(user = {}, options = {}) {
  ...
  return {
    ...
    accountLevelCurrentXpExact: progression.accountLevelCurrentXpExact,
    accountLevelNextXp: progression.accountLevelNextXp,
    accountLevelNextXpExact: progression.accountLevelNextXpExact,
    accountLevelSpanXp: progression.accountLevelSpanXp,
    accountLevelSpanXpExact: progression.accountLevelSpanXpExact,
    accountLevelRemainingXp: progression.accountLevelRemainingXp,
    accountLevelRemainingXpExact: progression.accountLevelRemainingXpExact,
...
module.exports = {
  ...
  normalizeOwnedFrameLevels,
  getFrameAssetIndexForLevel,
  isSelectedFrameUnlocked,
  isSelectedFrameOwned,
  isSelectedFrameSelectable,
