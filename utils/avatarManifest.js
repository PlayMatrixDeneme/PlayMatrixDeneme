...
function validateAvatarUrl(value = '', { storage = false } = {}) {
  const normalized = normalizeAvatarUrl(value, '');
  const allowed = !!normalized && AVATAR_SET.has(normalized);
  if (!allowed) {
  ...
  return storage ? sanitizeAvatarForStorage(normalized) : normalized;
}
...
function sanitizePublicAvatarForOutput(value = '') {
  return sanitizeAvatarForStorage(value) || DEFAULT_AVATAR;
}
