...
function buildAccountSyncPayload(userData = {}, extra = {}) {
  const merged = { ...userData, ...extra };
  const canonical = buildCanonicalUserState(merged, { defaultFrame: 0 });
  const balance = safeNum(merged.balance, 0);
  const monthlyActiveScore = Math.max(0, safeNum(merged.monthlyActiveScore ?? canonical.monthlyActiveScore, 0));
  return {
    balance,
    accountLevel: canonical.accountLevel,
    level: canonical.accountLevel,
    accountXp: canonical.accountXp,
    accountXpExact: canonical.accountXpExact,
    accountLevelScore: canonical.accountLevelScore,
    accountLevelScoreExact: canonical.accountLevelScoreExact,
    accountLevelProgressPct: canonical.accountLevelProgressPct,
    monthlyActiveScore,
