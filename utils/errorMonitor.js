...
function isBenignClientRuntimeError(err, safeCtx = {}) {
  const scope = cleanStr(safeCtx.scope || '', 160);
  const message = cleanStr(err?.message || '', 400).toLowerCase();
  if (scope === 'home.leaderboard.fallbackFetch' && (message.includes('load failed') || message.includes('abort') || message.includes('network'))) return true;
  return false;
}
...
async function captureClientError(body = {}, context = {}) {
  ...
  if (isBenignClientRuntimeError(err, safeCtx)) return null;
