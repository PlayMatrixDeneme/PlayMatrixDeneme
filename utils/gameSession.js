...
const ACTIVE_WAITING_STALE_MS = Math.max(120000, Number(process.env.ACTIVE_GAME_WAITING_STALE_MS || 180000));

function isStaleWaitingSession(status = '', lastActivityAt = 0, now = Date.now()) {
  return cleanStr(status || '', 24) === 'waiting'
    && safeNum(lastActivityAt, 0) > 0
    && now - safeNum(lastActivityAt, 0) > ACTIVE_WAITING_STALE_MS;
}
...
async function listActiveSessionsForUid(uid = '') {
  ...
  for (const doc of chessDocs.values()) {
    ...
    if (isStaleWaitingSession(status, lastActivityAt)) continue;
    sessions.push({
      gameType: 'chess',
      roomId: doc.id,
      status,
