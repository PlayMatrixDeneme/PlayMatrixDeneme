...
function shouldSuppressAmbientNotification(type = 'system', data = {}) {
  const safeType = normalizeNotificationType(type || 'system');
  const source = deriveNotificationSource({ type: safeType, data });
  return safeType === 'reward' || source.includes('reward') || source.includes('promo') || source.includes('spin') || source.includes('activity_pass');
}
...
async function createNotification({ uid = '', type = 'system', title = '', body = '', data = {}, read = false, idempotencyKey = '' } = {}) {
  ...
  if (shouldSuppressAmbientNotification(normalizedType, data)) {
    return shapeNotificationItem({
      uid: safeUid,
      type: normalizedType,
      title,
      body,
      ...
      createdAt: nowMs(),
      suppressed: true
    }, buildNotificationDocId({ uid: safeUid, type: normalizedType, title, body, idempotencyKey }));
  }
...
module.exports = {
  ...
  shouldSuppressAmbientNotification,
