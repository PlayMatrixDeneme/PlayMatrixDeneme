...
function getAccountXpExact(user = {}) {
  ...
  const explicitAccountXp = safeNum(user.accountXp ?? user.accountLevelScore ?? user.progression?.accountXp ?? user.progression?.accountLevelScore, Number.NaN);
  const explicitXp = safeNum(user.xp, Number.NaN);
  const explicitLevel = safeNum(user.accountLevel ?? user.level ?? user.progression?.accountLevel, Number.NaN);
  ...
  if (isCurrentProgressionVersion(user)) {
    ...
    for (const candidate of exactCandidates) {
      if (String(candidate ?? '').trim()) {
        const parsed = parseXpBigInt(candidate);
        if (parsed > bestExact) bestExact = parsed;
      }
    }
    if (Number.isFinite(explicitAccountXp) && explicitAccountXp >= 0) {
      const parsed = parseXpBigInt(explicitAccountXp);
      if (parsed > bestExact) bestExact = parsed;
    }
    if (Number.isFinite(explicitXp) && explicitXp >= 0) {
      const parsed = parseXpBigInt(explicitXp);
      if (parsed > bestExact) bestExact = parsed;
    }
    if (bestExact > 0n) return bestExact.toString();
...
function getAccountLevel(user = {}) {
  const accountXpCandidates = [
    user?.accountXpExact,
    user?.accountLevelScoreExact,
    user?.progression?.accountXpExact,
    user?.progression?.accountLevelScoreExact,
    user?.accountXp,
    user?.accountLevelScore,
    user?.progression?.accountXp,
    user?.progression?.accountLevelScore,
    user?.xp
  ];
  const hasXpSource = accountXpCandidates.some((candidate) => {
    if (candidate === null || candidate === undefined) return false;
    if (typeof candidate === 'string') return candidate.trim() !== '';
    const parsed = safeNum(candidate, Number.NaN);
    return Number.isFinite(parsed) && parsed >= 0;
  });
  if (hasXpSource) return getAccountLevelFromXp(getAccountXpExact(user));

  const explicitLevel = safeNum(user.accountLevel ?? user.level ?? user.progression?.accountLevel, Number.NaN);
  if (Number.isFinite(explicitLevel) && explicitLevel > 0) return normalizeLevel(explicitLevel);
  return 1;
}
...
function buildActivityResetState(currentPeriodKey = '', options = {}) {
  ...
  const periodKey = String(currentPeriodKey || '').trim();
  const activityPassClaimed = options.resetActivityPass === false
    ? undefined
    : (options.activityPassClaimed && typeof options.activityPassClaimed === 'object'
      ? { __periodKey: periodKey, ...options.activityPassClaimed }
      : (periodKey ? { __periodKey: periodKey } : {}));
  const payload = {
    monthlyActiveScore: 0,
    activityScore: 0,
    monthlyActivity: 0,
    ...
    lastActivityResetKey: periodKey || null
  };
  ...
  if (periodKey) payload.activityPassClaimedPeriodKey = periodKey;
