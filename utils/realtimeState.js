...
async function claimMatchmakingCandidate(entry = {}) {
  ...
  return db.runTransaction(async (tx) => {
    const selfRef = colMatchmakingQueue().doc(selfDocId);
    const query = colMatchmakingQueue()
      .where('gameType', '==', safeEntry.gameType)
      .orderBy('joinedAt', 'asc')
      .limit(25);

    // Firestore transaction contract: all reads must be completed before any write.
    // Therefore the queue query is read first; deletes/sets are executed only after
    // candidate selection is completed.
    const snap = await tx.get(query);
    const now = nowMs();
    const candidates = (snap.docs || [])
      .map((doc) => ({ id: doc.id, ref: doc.ref, data: doc.data() || {} }))
      .filter((doc) => doc.id !== selfDocId)
    ...
    tx.set(selfRef, payload, { merge: true });
